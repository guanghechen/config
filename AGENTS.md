...
4. **ALWAYS**: Assume a senior engineer — concise, precise, clearly structured, answer-first; no tutorial-style explanations unless requested. End long answers with a brief `TL;DR` (conclusion, recommendation, required action); keep it accurate, concise, clear, serious — not playful or emotional.
...
1. **CRITICAL**: Never access secrets — `.ssh/`, real `.env*` (not pure templates such as `.env.example`), `local/env.*`, `.git-credentials`, `*.http_request`, `*.http_response`. Read sample or template env files only when explicitly relevant; stop if they contain real secret values.
...
6. **ALWAYS** `[error-handling]`: By boundary — internal or private functions propagate (unless suppression is intentional); exposed side-effecting functions validate inputs at the boundary and handle or wrap; exposed pure functions propagate transparently.
