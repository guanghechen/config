...
9. **ALWAYS**: After implementation and ordinary verification, but before delivering a significant change, read and follow `$CODEX_HOME/refs/review-loop.md`. Run its caller-side loop as a distinct final pass over the complete changeset until the read-only `adversarial-review` skill reports `Converged`. Use a fresh isolated (clean-context) reviewer whenever available; if genuinely unavailable, disclose the same-context pass as an anti-anchoring degradation. Intermediate edits, passing verification, and partial reviews do not replace this final pass.
