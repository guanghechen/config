...
export const NodesRenderer: React.FC<INodesRendererProps> = (
  props,
) => {
  const { nodes } = props;
  const { viewmodel } = useNodeRendererContext();
  const rendererMap: Readonly<INodeRendererMap> = useStateValue(
    viewmodel.rendererMap$,
  );
...
class NodesRendererInner extends React.Component<IProps> {
  public override shouldComponentUpdate(
    ...
    return (
      !isEqual(props.nodes, nextProps.nodes) ||
      props.rendererMap !== nextProps.rendererMap
    );
  }
    ...
    return (
      <React.Fragment>
        {nodes.map((node, index) => {
          ...
          const Renderer: INodeRenderer =
            rendererMap[node.type] ?? rendererMap._fallback;
