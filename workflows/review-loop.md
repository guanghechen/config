...
本流程在 significant change 完成 implementation 和 ordinary verification 后、交付前执行。caller 是 budgets、attempt state、histories 与所有写操作的 single writer；只读的 `adversarial-review` core 独占审查判据、验证边界、disposition 与终态。下文的“停止”均指结束当前流程，并向用户报告证据、影响、已试方案、具体选项与推荐。
...
- reviewer 以完整 changeset 为主、latest delta（如有）为辅，独立复核 raw evidence，并返回 core 规定的 findings、dispositions 与适用时的终态。
- reviewer 完成 primary scope → `coverage: complete`；按 core 要求提前返回 `User decision required` → `coverage: stopped`；其他非预期中断均为 `invalid`。
- `Converged` 仅适用于 `coverage: complete` 且无未解决 material finding 或 blocking verification；`User decision required` 必须对应 `coverage: stopped`。
...
- `Converged` 且没有 `active` material finding → 交付；仍有 `active` finding → 停止并报告 caller state 与 core outcome 的冲突。
...
- caller 独占 histories、budgets、写操作与 workflow transition；core 独占审查判据、验证边界、disposition 与终态。caller 不得改判 core 的 tradeoff、non-material finding 或 blocking verification。
