...
9. **ALWAYS**: After implementation and ordinary verification, but before delivering a significant change, read and follow `$CLAUDE_CONFIG_DIR/workflows/review-loop.md`. Run its caller-side loop as a distinct, risk-proportionate final pass over the complete changeset until that loop converges — the reviewer reports `Clean` and no active finding remains; low-risk changes typically converge in a single pass. Use a fresh isolated (clean-context) reviewer whenever available; if genuinely unavailable, disclose the same-context pass as an anti-anchoring degradation. Intermediate edits, passing verification, and partial reviews do not replace this final pass.
