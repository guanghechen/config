...
printf "\n\e[94m  [setup miniforge] set environment variables...\e[0m\n"
export PYTHONIOENCODING=utf8
export PYTHONUTF8=1
export HOME_MINIFORGE="$HOME/.app/miniforge3"

if [ -d "$HOME_MINIFORGE" ]; then
  printf "\n\e[93m  [setup miniforge] miniforge is already installed. (skipped)\e[0m\n"
else
  ...
  if [ -f "${miniforge_setup_sh}" ]; then
    printf "\n\e[93m  [setup miniforge] $miniforge_setup_sh already exists. (skipped download)\e[0m\n"
  else
    printf "\n\e[94m  [setup miniforge] downloading...\e[0m\n"
    curl -L -O "https://github.com/conda-forge/miniforge/releases/latest/download/${miniforge_setup_sh}"
  fi

  printf "\n\e[94m  [setup miniforge] installing...\e[0m\n"
  printf "\n\nyes\n$HOME_MINIFORGE\nyes\n" | bash "${miniforge_setup_sh}" # should install at ~/.app/miniforge3
fi
export PATH=$HOME_MINIFORGE/bin:$PATH

### Setup conda
printf "\n\e[94m  [setup miniforge] setting up conda...\e[0m\n"
source "$HOME_MINIFORGE/etc/profile.d/conda.sh"
conda config --set auto_activate_base false

if conda env list | grep -q "^lemon\s"; then
  printf "\n\e[93m  [setup miniforge] the 'lemon' env is already created. (skipped)\e[0m\n"
else
  printf "\n\e[94m  [setup miniforge] creating 'lemon' env with conda...\e[0m\n"
...
if [ -f $ipython_config_path ]; then
  printf "\n\e[93m  [setup miniforge] $ipython_config_path is already exist. (skipped).\e[0m\n"
else
  printf "\n\e[94m  [setup miniforge] setting up ipython...\e[0m\n"
