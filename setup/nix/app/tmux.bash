...
case "$prefer_tmux_version" in
  latest | nightly)
    if ! command -v brew &>/dev/null; then
      printf "\e[93mHomebrew is not installed (skipped tmux installation)\e[0m\n"
    elif [ -n "${HOME_HOMEBREW:-}" ] && [ ! -w "$HOME_HOMEBREW/var/homebrew/locks" ]; then
      printf "\e[93mHomebrew lock directory is not writable (skipped tmux installation)\e[0m\n"
    elif [ "$prefer_tmux_version" = "nightly" ]; then
