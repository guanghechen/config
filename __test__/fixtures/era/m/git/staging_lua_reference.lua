--- Frozen staging calculations before the Rust migration; differential oracle only.
---@diagnostic disable-next-line: unused-local
local __module_name__ = "__test__.fixtures.era.m.git.staging_lua_reference" ---@type string
...
-- The following batch operations freeze the loops previously owned by buffer.lua.
---@param hunks                         era.m.git.Hunk[]
---@param top                           integer
---@param bot                           integer
---@param partial                       boolean
---@return era.m.git.Hunk[]
function M.select_hunks(hunks, top, bot, partial)
  local selected = {}
  for _, hunk in ipairs(hunks) do
    if partial then
      local intersected = M.intersect(hunk, top, bot)
      if intersected then
        selected[#selected + 1] = intersected
      end
    elseif M.touches(hunk, top, bot) then
      selected[1] = hunk
      break
    end
  end
...
---@param hunks                         era.m.git.Hunk[]
---@return era.m.git.Hunk[]
function M.invert_hunks(hunks)
  local inverted = {}
  for _, hunk in ipairs(hunks) do
    inverted[#inverted + 1] = M.invert(hunk)
  end
  table.sort(inverted, M.less)
  return inverted
end
...
---@return era.m.git.Hunk[]
---@return boolean
function M.without_range(hunks, top, bot)
  local untouched, touched = {}, false
  for _, hunk in ipairs(hunks) do
    if M.touches(hunk, top, bot) then
      touched = true
    else
