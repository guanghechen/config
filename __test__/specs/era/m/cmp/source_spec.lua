...
t:test("keyword sources preserve the suffix while path edits replace their explicit filename range", function()
  ...
  t.assert_eq(3, item.textEdit.range["end"].character, "dictionary prefix end")
...
t:test("ordinary paths use the request buffer directory", function()
  ...
  file:close()
  cwd = root
  t:patch_table(vim.uv, "fs_open", function()
    error("enumeration must not read candidate contents")
  end)
  local canonical_filepath = assert(vim.uv.fs_realpath(filepath))

  for _, line in ipairs({ '"sam', '"./sam' }) do
    local result = complete(line, "lua", nil, vim.fs.joinpath(buffer_dir, "main.lua"))
    t.assert_true(
      vim.iter(result.items):any(function(item)
        if item.label == "sample.lua" and item.data.era_cmp.source == "path" then
          t.assert_eq(canonical_filepath, item._era_cmp_path, "lazy documentation path")
          return true
        end
        return false
      end),
