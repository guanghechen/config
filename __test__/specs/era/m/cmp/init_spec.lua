...
local accept_base = nil ---@type era.m.cmp.editor.IBase|nil
t:patch_table(package.loaded, "era.m.cmp.accept", {
  cancel = function() end,
  apply = function(completed, record, base)
    accept_calls = accept_calls + 1
    accept_base = base
    if accept_result then
      local item = completed.user_data.era_cmp.item
      require("era.m.cmp.editor").apply(
        base,
        item.textEdit.range.start,
        item._era_cmp_suffix_bytes or 0,
...
local bridge_resolve_request = nil ---@type { item: table, callback: function, line: string, cancellations: integer }|nil
...
t:patch_table(package.loaded, "era.m.cmp.bridge", {
  ...
  resolve = function(item, callback)
    bridge_resolve_request = {
      item = item,
      callback = callback,
      line = vim.api.nvim_get_current_line(),
      cancellations = bridge_resolve_cancel_count,
    }
    return function()
...
t:test("mid-token preview owns replacement suffix lifecycle", function()
  ...
  t.assert_eq("future", assert(accept_base).line, "original preview text")
  t.assert_eq(3, accept_base.col, "original cursor")
...
t:test("steady-state selection replaces the preview with one buffer write", function()
  ...
  t:patch_table(vim.api, "nvim_buf_set_text", function(target_bufnr, ...)
    if target_bufnr == bufnr then
      writes = writes + 1
    end
    return set_text(target_bufnr, ...)
  end)
  callbacks.actions.move(bufnr, 1)
...
t:test("leaving insert mode commits the preview without accepting side effects", function()
  local bufnr = vim.api.nvim_get_current_buf() ---@type integer
  vim.api.nvim_set_current_line("future")
  vim.api.nvim_win_set_cursor(0, { 1, 3 })
  publish({ completion_item("futile", 3) })
  ...
  t.assert_eq("futile", vim.api.nvim_get_current_line(), "committed preview")
  t.assert_false(popup_visible, "hidden popup")
end)

t:test("cursor movement dismisses stale preview state without rewinding user movement", function()
  ...
  callbacks.CursorMovedI({ buf = bufnr })
  t.assert_false(popup_visible, "stale view dismissed")
  t.assert_eq("futile", vim.api.nvim_get_current_line(), "moved preview remains committed")
  t.assert_eq(2, vim.api.nvim_win_get_cursor(0)[2], "user cursor preserved")
end)
...
t:test("selection documentation is generation guarded", function()
  ...
  item._era_cmp_origin = {
    client_id = 81,
    context = { line = "vall", col = 3, filetype = "lua" },
    item = { label = "value" },
    start_col = 0,
    suffix_bytes = 0,
    target_start_col = 0,
  ...
  t.assert_eq("vall", request.line, "resolve starts before preview mutation")
...
t:test("selection subscribes before cancelling documentation for the same candidate", function()
  local bufnr = vim.api.nvim_get_current_buf()
  local item = completion_item("value")
  item._era_cmp_origin =
    { client_id = 81, context = { line = "vall", col = 3, filetype = "lua" }, item = { label = "value" } }
  vim.api.nvim_set_current_line("vall")
  vim.api.nvim_win_set_cursor(0, { 1, 3 })
  publish({ item })
  local previous = assert(bridge_resolve_request)
  local cancellations = bridge_resolve_cancel_count
  callbacks.actions.move(bufnr, 1)
  local current = assert(bridge_resolve_request)
  t.assert_true(current ~= previous, "new view subscription")
  t.assert_eq(cancellations, current.cancellations, "new listener joins before the old one cancels")
  t.assert_eq(cancellations + 1, bridge_resolve_cancel_count, "old view subscription released")
  t.assert_eq("vall", current.line, "request precedes preview insertion")
end)

t:test("path documentation updates only its selected generation without LSP resolution", function()
  local requests = {}
  local cancelled = 0
  t:patch_table(require("era.m.cmp.path_documentation"), "request", function(filepath, callback)
    requests[#requests + 1] = { filepath = filepath, callback = callback }
    return function()
      cancelled = cancelled + 1
    end
  end)
  local item = completion_item("value.lua")
  item._era_cmp_path = "/fixture/value.lua"
  vim.api.nvim_set_current_line("vall")
  vim.api.nvim_win_set_cursor(0, { 1, 3 })
  documentation_update = nil
  bridge_resolve_request = nil
  publish({ item })
  t.assert_eq(item._era_cmp_path, requests[1].filepath, "selected file owns the read")
  t.assert_nil(bridge_resolve_request, "local documentation bypasses LSP resolve")
  requests[1].callback("```lua\nreturn 1\n```")
  t.assert_true(documentation_update.text:find("return 1", 1, true) ~= nil, "content reaches the owned popup")

  publish({ item })
  local pending = requests[2]
  publish({ completion_item("other") })
  documentation_update = nil
  pending.callback("obsolete")
  t.assert_eq(1, cancelled, "replaced documentation request is cancelled")
  t.assert_nil(documentation_update, "obsolete path content cannot replace a newer view")
end)
