...
t:test("respects an explicit prefix edit and normalizes snippet text", function()
  local line = "foobar"
  local bufnr, params = context(line)
  ...
  t.assert_eq(0, item._era_cmp_suffix_bytes, "explicit edit keeps the suffix")
...
t:test("drops a completion response after the trigger context changes", function()
  ...
  t.assert_eq(1, publications, "only the terminal stale result is delivered")
  Bridge.cancel(bufnr)
  vim.api.nvim_buf_delete(bufnr, { force = true })
end)
...
t:test("merges prompt upstream results into the initial snapshot and refreshes later results", function()
  ...
  Bridge.complete(params, function(_, value)
    initial = value
    publications[#publications + 1] = value
  end)
  ...
  t.assert_eq(1, #publications, "one initial publication")

  responses[66](nil, { items = { { label = "variable" } } })
  t.wait_until(function()
    return #publications == 2
  end, 1000, "late publication without a second complete call")
  local final = publications[2]
...
t:test("retains matching upstream candidates while a token grows", function()
  ...
  t.assert_eq(2, initial.items[1]._era_cmp_suffix_bytes, "explicit replacement suffix")
  ...
  for _, value in ipairs({ "lf", "lfu" }) do
    ...
    t.assert_eq(2, extended.items[1]._era_cmp_suffix_bytes, "retargeted explicit suffix for " .. value)
...
t:test("completion deadline retains the local-first result and cancels stalled clients", function()
  ...
  t.wait_until(function()
    return cancel_count == 1 and #publications == 2
  end, 2500, "deadline publication")
