...
t:test("cmdline position renders when cursor movement changes layout", function()
  ...
  state.special = nil
  state.ghost = "tail"
  cmdline.pos({ event = "cmdline_pos", args = { 5, 1 } })
  t.assert_nil(state.ghost, "cursor movement clears completion ghost text")
  t.assert_eq(3, renders, "ghost removal redraws the rendered content")
  t.assert_eq(0, cursor_updates, "ghost removal cannot use the cursor-only path")

  cmdline.pos({ event = "cmdline_pos", args = { 6, 1 } })
  t.assert_eq(3, renders, "subsequent cursor movement preserves the fast path")
  t.assert_eq(1, cursor_updates, "cursor-only updates resume after clearing ghost text")
end)
...
t:test("programmatic cmdline sync reuses the active render surface", function()
  local cmdline, states = setup()
  local rendered = nil ---@type era.dressing.ui_attach.cmdline.IState|nil
  states.cmdline[1] = {
    level = 1,
    firstc = ":",
    prompt = "",
