...
t:test("RPC-only Yui handles real focus and Insert events without attaching a UI", function()
  ...
  t.assert_eq(0, eval("return im_rpc_state.captures + im_rpc_state.restores"), "headless events do not own focus")

  vim.rpcrequest(channel, "nvim_exec_autocmds", "FocusGained", {})
  vim.rpcrequest(channel, "nvim_exec_autocmds", "FocusGained", {})
  t.assert_eq(1, eval("return im_rpc_state.captures"), "explicit focus selects English once")
  t.assert_eq("english", eval("return im_rpc_state.source"))

  input("i", "i")
  eval("im_rpc_state.source = 'non_english.editing'")
  input("<Esc>", "n")
  t.assert_eq(2, eval("return im_rpc_state.captures"), "actual InsertLeave captures the source")
  t.assert_eq("english", eval("return im_rpc_state.source"))
  input("i", "i")
  t.assert_eq(1, eval("return im_rpc_state.restores"), "actual InsertEnter restores the source")
  ...
  input("<Esc>", "n")
  input("i", "i")
  t.assert_eq(3, eval("return im_rpc_state.captures + im_rpc_state.restores"), "unfocused mode changes do no work")
