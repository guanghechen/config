...
- 面板改为固定标题栏、可滚动属性区与固定底部操作栏；色板按列对齐，线宽/手绘/填充/文字对齐使用 SVG 分段按钮。文字、连线、变换设置可折叠，说明文字集中于 Selection tips。
- Edge 153 / Playwright CLI 实测矩形单选默认高度约 542px，无需内部滚动；1280×900、768×700、390×844、320×568 下的单选/多选与展开状态无横向溢出，较矮视口只滚动中间属性区。检查明暗主题、色板列位置和输入控件对齐。
- 实际操作覆盖颜色与 No fill、线宽/手绘/填充图案、字号 Enter 提交和 Escape 取消、字体/粗体/对齐/自动尺寸、角度/旋转/翻转、标签编辑、锁定/隐藏、重复/删除/undo、对齐/分组，以及仅连线选区的分组、路线/箭头/线型和折点增删/重置。面板内 Enter/Space 不触发画布编辑，折叠后再展开仍保留锁定禁用状态。
- 使用 Node 24.16.0 运行 typography、pose、sketch、protection、stacking、organization、connectors 七组测试，51 项通过，0 失败/跳过。`pnpm format` 为 0 errors / 54 项既有 warnings；前端 TypeScript 仍为相同的 8 项既有诊断，无新增。没有新增依赖。


## 2026-09-12：模块组织与宿主隔离

- `View.tsx` 保留原有 YOZ 入口，独立编辑器从 `Whiteboard.tsx` 导出。新增静态宿主接口；文件、草稿、站点主题、Markdown、Monaco、workspace、URL 与 Vite 通知由 `host/` 适配，核心模型与文件格式保持不变。
- 输入按手势、键盘、剪贴板拆分；渲染、文件生命周期、编辑会话、工具栏与属性面板分别组织。样式按模块拆分，公共编辑器只使用 `--wb-*`，宿主样式单独加载。
- 新增 4 项 architecture tests：限制宿主业务依赖、遍历独立入口含 lazy imports 的依赖图、检查内部依赖环、检查样式对宿主变量/布局的依赖。既有引用资源并发测试改为通过文件接口注入 loader，继续验证共享请求、过期响应、取消及重新订阅。
- 独立浏览器 fixture 不加载 SiteContext、路由、登录或站点公共样式。实际完成绘图、键盘移动/undo、样式/旋转、标签与文本编辑、分组、连线、阅读模式和 SVG 导出；浏览器资源记录确认没有 YOZ 服务、Provider 或 Monaco 请求。
- YOZ fixture 验证单选/多选、锁定/隐藏、编辑/重复/删除/undo、引用与内联 Markdown、Monaco、站点主题、演示、文件保存和富内容 SVG 导出。源文件变化时保留本地草稿，条件保存拒绝过期版本，显式重新加载采用外部版本。
- 另存创建后打开新文件，系统剪贴板复制/粘贴正常；1280、768、390、320px 工具栏保持单行，面板无横向溢出。独立入口通过 `style` 在 480×600 容器内挂载，属性面板按容器高度滚动。
- Node 24.16.0：完整相关回归 123 项通过、0 失败/跳过。前端 TypeScript 与修改前保持同样 8 个诊断位置和错误码（union 打印顺序可变），无新增诊断；`pnpm format` 为 0 errors / 54 项既有 warnings。未增加依赖，formatter 对 lockfile 的纯格式变化按 YAML 语义比较后恢复。
- 固定 1000 节点 + 1000 连线场景（含文字、曲线、旋转/镜像），macOS / Edge 153 headless / 1920×1080 / DPR 1 / Vite development：reading 的 pan、zoom、drag、resize、rotate 与 overview 的 pan、zoom 共七项，40 帧预热、240 帧采样，平均 59.5–60.0 FPS，P95 16.7–16.8ms，最大 33.4ms；富内容与字体就绪为 0.23–0.80 秒。旋转项确认实际改变卡片姿态。


## 2026-09-12：重组后的审查与修复

- 复现并修复实例串扰：两个白板同时有选区时，在第二个白板按 Delete 曾删除两边的选区；白板外按 R 曾切换所有白板。键盘、剪贴板与阻塞对话框检查现在限定到所属白板；保留按钮的 Enter/Space 原生操作。
- 复现并修复保存/导入竞态：保存期间导入会改变共享 generation，旧保存结束后曾永久停在 Saving。保存现在按组件会话判断结果归属，保留新文档并更新 revision；被后续导入取消的重载也会清除 loading。没有草稿存储时仍保护未保存修改。
- 复现并修复容器越界：480×600 白板中的内容编辑器曾右溢出 276px、下溢出 230px。编辑器与右键菜单现在使用白板 viewport，编辑器的原生拖动缩放也受容器边界限制。
- 初次文件读取同步抛错、返回空响应，以及可选通知订阅失败均可恢复；文件服务失败不会卸载白板或留下永久加载层，通知失败保留轮询。
- 新增 `tests/whiteboard-lifecycle.browser.test.mjs`，沿用仓库 Node + 已安装 Chromium/CDP 的测试方式，覆盖八组实际 React/DOM 场景。浏览器 profile 位于 Vite root 外，避免浏览器自身文件触发热重载；测试只使用内存文件服务和临时目录。
- 修复后完整回归 124 项通过，0 失败/跳过；其中浏览器测试实际使用 Edge/Chromium，验证双实例键盘/剪贴板/演示隔离、保存与重载竞态、离开保护、宿主失败恢复和容器边界。`pnpm format` 为 0 errors / 54 项既有 warnings，前端 TypeScript 保持相同 8 个诊断位置和错误码，无新增；测试 TSX fixture 已纳入 ESLint project。最后一轮复核未发现新的 material finding。
