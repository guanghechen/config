...
- 每次有效请求和组件通知都可以安排发布；同一轮的发布合并为一次，`draw_interval` 默认 16ms，设为 0 时只合并当前轮。首次组件完成可以立即安排发布，避免启动占位内容阻挡首个就绪结果；之后首沿与固定截止时间的尾沿控制布局频率，持续请求不会让最后的更新饿死。interval 到期后，纯组件通知直接布局并调用 `on_fulfilled`；含显式请求时按下述队列顺序处理。
- 组件完成但文本未变时跳过结果回调；显式 `refresh()` 仍保证一次结果回调，确保切回某个 tabline 等场景能恢复目标。有效 owner 的发布只排版已有快照；owner 过期时按下述规则重新请求数据。组件通知与绘制的 deadline 均复用 queue 的单个 unref timer，已进入到期快照的 callback 仍会检查取消或替换状态。
- 显式 refresh 的发布排在已入队的组件工作之后，使同轮 warm 数据与发布合并，避免先发布跨 scope 的空布局。发布只等待队列中的同步调用返回，不等待未完成的 Future；仍无可见数据时正常发布空内容。节流到期若合并了显式请求，也遵循这个顺序。新的显式请求仅在 owner 变化时替换旧发布任务；同 scope 请求复用排队位置，避免持续输入让发布不断后移。截止时间仍由上次实际发布决定。
- 发布保留触发它的 readonly context；真正写入前再次校验 owner。窗口、buffer、路径或 cwd 已变化时，丢弃这次发布并刷新当前 scope，不让旧发布先清空新目标。
- 首次组件完成、发布任务替换和 dispose 时立即取消旧的排队句柄，避免废弃发布占用后续 cold turn。组件完成通知继续遵守原有首沿与尾沿规则。
