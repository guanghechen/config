...
| 功能     | TabType              | 描述                                                                                        |
|:---------|:---------------------|:--------------------------------------------------------------------------------------------|
| Git Diff | `diffview_workspace` | 查看 staged/unstaged 文件变更，左侧显示文件树，右侧显示 side-by-side diff                   |
...
| commits  | `pane/commits.lua`  | commit 列表            | commits view  |
...
| `Fdiffviewtogglecommits`   | 切换 commits 面板显示（commits view）                |
| `Fdiffviewtogglefiles`     | 切换 files 面板显示（workspace）                     |
...
每个 view 独立维护自己的状态，存储在 `view/*/state.lua` 中。使用 `stl.c.Observable` 实现响应式更新。
...
Git Log 视图 (`diffview_commits`) 支持分页浏览 commits：
