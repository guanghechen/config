...
#define S(d,b) (1.0 - smoothstep(b,antialiasing(3.0),d))
...
void mainImage(out vec4 fragColor, in vec2 fragCoord) {
    ...
    p.y += iTime * 0.02;
    vec3 pattern = drawBelt(p, vec3(0.0), 4.0);
    pattern = drawGearsAndItems(p, pattern, 4.0);
    float opacity = clamp(pattern.r, 0.0, 1.0) * 0.11 * mask;
    fragColor.rgb = mix(fragColor.rgb, vec3(0.29, 0.34, 0.42) * fragColor.a, opacity);
}
