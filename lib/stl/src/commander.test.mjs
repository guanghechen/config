...
function createReporter() {
  return new Reporter({ prefix: 'test', flight: { date: false, color: false } })
}
...
describe('Command', () => {
  ...
  describe('constructor', () => {
    it('creates command with name and reporter', () => {
      const reporter = createReporter()
      const cmd = new Command('my-cli', reporter)
      assert.equal(cmd.name, 'my-cli')
      assert.equal(cmd.reporter, reporter)
    })
  })
  ...
  describe('fluent API', () => {
    it('supports method chaining', () => {
      const cmd = new Command('cli', createReporter())
  ...
  describe('argument', () => {
    it('parses required argument', () => {
      const cmd = new Command('cli', createReporter()).argument('<file>', 'File')
    ...
    it('parses optional argument with default', () => {
      const cmd = new Command('cli', createReporter()).argument('[format=json]', 'Format')
      const { args } = cmd.parse([], {})
    ...
    it('parses variadic argument', () => {
      const cmd = new Command('cli', createReporter()).argument('[...files]', 'Files')
      const { args } = cmd.parse(['a.txt', 'b.txt'], {})
  ...
  describe('option', () => {
    it('parses boolean option', () => {
      const cmd = new Command('cli', createReporter()).option('-f, --force', 'Force')
      assert.equal(cmd.parse(['--force'], {}).opts.force, true)
      assert.equal(cmd.parse(['-f'], {}).opts.force, true)
    })

    it('parses string option', () => {
      const cmd = new Command('cli', createReporter()).option('-c, --config <path>', 'Config')
  ...
  describe('option default & env', () => {
    it('uses default value', () => {
      const cmd = new Command('cli', createReporter()).option('-p, --port <n>', 'Port', { type: 'number', default: 3000 })
      assert.equal(cmd.parse([], {}).opts.port, 3000)
    })

    it('uses env value', () => {
      const cmd = new Command('cli', createReporter()).option('-p, --port <n>', 'Port', { type: 'number', env: 'PORT' })
      assert.equal(cmd.parse([], { PORT: '8080' }).opts.port, 8080)
    })

    it('follows priority: default < env < argv', () => {
      const cmd = new Command('cli', createReporter()).option('-p, --port <n>', 'Port', {
        type: 'number',
        default: 3000,
        env: 'PORT',
      })
    ...
    it('coerces boolean env values', () => {
      const cmd = new Command('cli', createReporter()).option('-f, --force', 'Force', { env: 'FORCE' })
  ...
  describe('negatable option', () => {
    it('negates with --no-prefix', () => {
      const cmd = new Command('cli', createReporter()).option('--color', 'Color')
  ...
  describe('option syntax', () => {
    ...
    it('converts kebab-case to camelCase', () => {
      const cmd = new Command('cli', createReporter()).option('--dry-run', 'Dry run')
  ...
  describe('built-in options', () => {
    ...
    it('has --version when set', () => {
      const cmd = new Command('cli', createReporter()).version('1.0.0')
  ...
  describe('strict mode', () => {
    ...
    it('allows unknown option when disabled', () => {
      const cmd = new Command('cli', createReporter()).strict(false)
      const { diagnostics } = cmd.parse(['--unknown'], {})
  ...
  describe('run', () => {
    it('shows help on --help', async () => {
      const cmd = new Command('cli', createReporter()).action(async () => {
    ...
    it('shows version on --version', async () => {
      const cmd = new Command('cli', createReporter()).version('1.0.0').action(async () => {
        throw new Error('Should not execute')
      })
