...
describe('Reporter', () => {
  ...
  it('falls back invalid level to info', () => {
    const r = new Reporter({ level: 'invalid' })
  ...
  it('pushes and pops prefixes', () => {
    const r = new Reporter({ prefix: 'app' })
    r.mock()
    r.info('1')
    r.attach('theme').info('2')
    r.attach('apply').info('3')
    r.detach().info('4')
    r.detach().info('5')
    const logs = r.collect()
  ...
  it('protects initial prefix from detach', () => {
    const r = new Reporter({ prefix: 'app' })
    r.mock().detach().detach().info('test')
    assert.deepEqual(r.collect()[0].prefixes, ['app'])
  })
  ...
  it('allows detach without initial prefix', () => {
    const r = new Reporter()
    r.mock().attach('sub').info('1').detach().info('2').detach().info('3')
    const logs = r.collect()
