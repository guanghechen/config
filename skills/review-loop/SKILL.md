...
Run this as a distinct final pass. The caller is the single writer for loop state, fixes, verification, and transitions; the `adversarial-review` skill remains read-only and owns review judgment.
...
- Start a new non-fork subagent, require the `adversarial-review` skill, and keep it read-only. Non-fork subagents start with fresh isolated context. Never resume or reuse the implementer thread or a prior reviewer, and never use a conversation fork while a fresh reviewer is available.
