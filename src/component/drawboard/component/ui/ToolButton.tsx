  ...
  return (
    <button
      ...
      className={cn(
        ...
        sizeClasses[size],
        {
          // Active states - blue background with black stroke icons
          'bg-blue-500/90 text-white': isActive && variant === 'primary',
          'bg-gray-700 text-white': isActive && variant === 'secondary',
          'bg-red-500/90 text-white': isActive && variant === 'danger',

          // Inactive states - very subtle hover
          'text-gray-600 hover:bg-gray-100/70': !isActive && !disabled && variant === 'primary',
          'text-gray-500 hover:bg-gray-50/70': !isActive && !disabled && variant === 'secondary',
          'text-red-500 hover:bg-red-50/70': !isActive && !disabled && variant === 'danger',
    ...
    >
      {isLoading ? (
        ...
        <Icon
          className={cn(iconSizes[size], 'flex-shrink-0', {
            'stroke-black': isActive && variant === 'primary',
          })}
        />
      )}
      ...
      {/* Tooltip matching Excalidraw's style */}
      <div className="absolute top-full mt-3 left-1/2 -translate-x-1/2 z-50 pointer-events-none opacity-0 group-hover:opacity-100 transition-opacity duration-150 delay-500">
        <div className="rounded-md bg-gray-800 px-2 py-1.5 text-xs text-white whitespace-nowrap shadow-lg">
          <div className="font-medium">{label}</div>
          {displayShortcut && !numericKey && (
            <div className="text-gray-300 mt-0.5 text-[10px]">{displayShortcut}</div>
          )}
          {/* Tooltip arrow */}
