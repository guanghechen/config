...
export const useDragAndDrop = (props: IProps): IResult => {
  ...
  const handleDragStart = useEventCallback((node: IGraphNode, screenX: number, screenY: number) => {
  ...
  const handleDragMove = useEventCallback((screenX: number, screenY: number) => {
    ...
    if (potentialTarget && potentialTarget.position) {
      ...
      const targetScreenPos = worldToScreen(potentialTarget.position.x, potentialTarget.position.y)
  ...
  const handleDragEnd = useEventCallback((screenX: number, screenY: number) => {
