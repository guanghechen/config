...
import { JsonlComposer } from './composer'
import { useScrollToTop } from './useScrollToTop'

const JsonlContainer: React.FC = () => {
  ...
  const { data, error } = useFileResult<IJsonlFileData>(workspace, filepath, tick)
    ...
    <div className="w-full pt-8">
      ...
      {!!data && (
        <div className="relative w-full">
          <JsonlComposer content={data?.content} />
        </div>
      )}
      <button
        onClick={scrollToTop}
...
export default React.memo(JsonlContainer, () => true)
