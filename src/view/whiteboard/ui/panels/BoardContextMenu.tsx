...
export const BoardContextMenu: React.FC<{
...
}> = ({ viewport, position, selected, store, close, copy, paste, edit }) => {
  ...
  const top = Math.max(8, Math.min(position.y, viewport.height - 520))
  React.useEffect(() => {
  ...
  return (
    <div
      ...
      style={{
        left: Math.max(8, Math.min(position.x, viewport.width - 268)),
        top,
        maxHeight: Math.max(0, viewport.height - top - 8),
      }}
      onKeyDown={event => {
