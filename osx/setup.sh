...
source ~/.config/guanghechen/osx/setup/bot/font-maple.sh
...
if [ -z "$HOME_HOMEBREW" ] || [ -w "$HOME_HOMEBREW/var/homebrew/locks" ]; then
  ...
  source ~/.config/guanghechen/osx/setup/bot/homebrew-patch.sh
...
source ~/.config/guanghechen/nix/setup/env/bun.sh
printf "\e[92m  [setup bun] done.\e[0m\n"

### Setup node
printf "\n\e[96m  [setup node] preparing...\e[0m\n"
source ~/.config/guanghechen/nix/setup/env/node.sh
