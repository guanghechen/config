...
- `source:same_content(other)` 比较同一 DataHandle 内的 NodeId 集合、parent 关系与完整 NodeData，包含 opaque fields。
  加载状态、children 完整性、请求进度和 sibling 顺序不参与比较；不同 owner 始终返回 false。
  比较利用持久化索引跳过共享分支，只检查变动节点。按行缓存还须独立校验 layout revision，不能据此跨布局复用。
