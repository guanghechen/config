...
State 的 `selection_purpose` 是上层拥有含义的不透明字符串，core 不解释 copy/cut 等业务用途。
上层 native action 在同一次 owner 提交中改变 selection 与用途，snapshot/StateStatus 原子发布两者；
选区精确为空且未被任务锁定时清除用途。切换用途本身不刷新 selection stamp 或 selection revision。
`state:status()` 提供当前已提交的 `selection_purpose`、锁和 revisions，`state:display()` 按需读取同一 publication 的
显示选项及 state revision，提交修改时使用这对值，不能把旧选项和另一次读取的新 revision 混用。Surface 的旧 frame
继续描述自己的布局，任务取得锁后应读已提交状态，不能用尚未重绘的用途解释新 Ready 源项。
...
- Lua attach 的 `bufname` 指定专用正文 buffer 的最终名称，默认使用 `treeview://` 身份。
  创建时只命名一次，避免二次命名在 Neovim 中留下旧名称的 unloaded buffer；detach 释放该正文 buffer。
...
Provider 自身的读取或能力失败使用 `ProviderError`，保留其操作与平台错误信息。
Lua provider 的非结构化抛错或字符串形式的 error 使用 `ProviderError`；不符合分页协议的返回值或非法 error payload 仍为 `InvalidUpdate`。
错误统一为 `InvalidUpdate / MissingNode / Stale / Busy / Disposed / ResourceLimit / ProviderError`；普通输入错误返回
...
Lua `state:lock_selection(deadline_ms?, selection_revision?)` 可传入先前查询捕获的 selection revision，
由 native owner 在取得锁时原子校验；省略时使用调用时的当前版本。补载引起的后续 revision 推进不等于用户改选。
