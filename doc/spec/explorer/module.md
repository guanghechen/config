...
已落地的技术协议与默认入口行为以 [Explorer Design](../../../spec/design/feat/explorer.md) 为准；
本文保留讨论与未决提案，不再作为旧 Lua 实现的技术契约。
“沿用现有能力”不构成额外的隐式规范；实现与验收以本文展开的行为为准。
文中的“草案”表示推荐方案尚未最终确认，技术协议未定稿不以引用其他文档代替。

## 已确认的架构方向

- 数据、状态、树计算和文件操作放在现有 `yoz` crate 的 `rust/yoz/src/ux/`。
...
三层是 `rust/yoz/src/ux/` 内的模块职责，不分别创建 crate：

- Filetree 决定资源 identity、结构事实、metadata、目录加载/刷新及文件领域的排序和状态聚合；
  通过通用 data owner 发布唯一权威 topology，技术边界见 [Filetree 契约](../../../spec/design/filetree.md)。
...
这些通知不自动生成标记版本；显式清空和业务成功项清理由 Explorer 单独提交。
目录内部分成功遵循 [Filetree 逐项结果契约](../filetree/module.md#已确认目录内部结果与选择清理)。
...
- Copy/Move to directory 的目录输入属于任务准备阶段：显示输入框前固定光标源项并锁定逻辑选区。
  输入期间光标移动、watch 更新或原源项消失，都不能改为操作另一个文件；取消输入释放准备状态与锁。
...
Copy/move 采用逐项处理方式；对允许覆盖的目标冲突，逐项询问是否 override。
...
Copy 的同名目录合并与逐文件冲突处理遵循
[Filetree Copy 契约](../filetree/module.md#已确认copy-的目录合并与冲突)。
Move 遇到同名目标目录时遵循
[Filetree Move 契约](../filetree/module.md#已确认move-的目录冲突)。
普通文件与普通目录的同名目标冲突遵循
[Filetree 类型冲突契约](../filetree/module.md#已确认文件与目录的类型冲突)。
Symlink 操作遵循 [Filetree symlink 契约](../filetree/module.md#已确认symlink-操作与覆盖)，
覆盖确认的有效性遵循 [Filetree 覆盖校验](../filetree/module.md#已确认覆盖确认的适用范围)。
...
- 目录内部也按成功部分取消选择、失败或跳过部分保留，遵循
  [Filetree 逐项结果契约](../filetree/module.md#已确认目录内部结果与选择清理)。
- 用户拒绝覆盖：该项为 skipped，保留选中和原 mode。
- 已成功清理的项不会因其他项失败而重新加入选区；cleanup Stale 时成功项可以暂留，不能据此重放 IO。
- “Failed”不代表本项完全没有副作用；单文件中途失败不算成功，保留选择，不自动重试。
  临时输出的 IO 收尾由 provider 技术设计确定。
...
- Normal 文件修改优先使用逻辑选区，无选区时回退到光标项；Visual `d` / `oa` 使用临时范围。
- `l` / `<CR>` / 双击使用光标项；显式打开选区和 Normal 路径导出仍优先使用逻辑选区，无选区时回退到光标项。
...
| `l` / `<CR>` / `<2-LeftMouse>`（双击） | 光标在目录时切换展开；文件始终打开光标项，并保留逻辑选区             |
...
| `d`       | 确认后删除临时范围内的最外层子树，保留原有无关标记及用途               |
| `oa`      | 导出临时范围内最外层子树的位置，保留逻辑选区                           |
| `q`       | 关闭当前 Explorer pane                                                  |

`l` / `<CR>` 的光标目标与 Visual `d` / `oa` 的临时范围行为已确认；最终契约见
[Explorer Design](../../../spec/design/feat/explorer.md)。显式 `o<CR>` 继续使用逻辑选区。
...
- Visual `d`、`oa` 保留直接消费临时范围的行为，不以临时范围覆盖逻辑选区。
...
2. 已确认：同名目录合并、file/directory 类型冲突和 symlink 行为见上文目标冲突及 Filetree 契约。
3. 已确认：目录内部成功部分取消选择、失败或跳过部分保留；具体规则见 Filetree 逐项结果契约。
