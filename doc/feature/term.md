...
1. The Terminal widget lives at `lua/eve/ux/widget/terminal.lua` and exposes the shared widget interface so other modules can focus / toggle it.
   - A floating window is centred in the editor with a rounded border (`relative = "editor"`) and the following window styling:
     ...
     ```
   - The window initially opens with an internal mask buffer (`filetype = std.filetype.TERM_MASK`) to keep the layout stable while the real terminal buffer is attached.
...
2. Each terminal tab runs in a dedicated buffer created by `eve.term.create`:
   - Buffer options match expectations for pseudo terminals:
     ```lua
     vim.bo[bufnr].buflisted = false
     vim.bo[bufnr].filetype = std.filetype.TERM
   ...
   - `TermClose` autocmds translate vim events back through `eve.term.on_closed` so the store stays consistent even when the user exits the program inside the terminal.

3. The terminal winbar mirrors the Notepad UX and keeps long lists approachable:
   - `lua/eve/ux/nvimbar/component/term.lua` exposes `items(position)` and `add_button(position)`. Legacy consumers can still call `term.terms`.
...
### Data Model (`lua/eve/builtin/term.lua`)
...
- Every action routes notifications through `ark.reporter` and ensures `dot.status.dirtier_termline` is marked so the widget winbar stays current.
...
- Observes `eve.term.o_termuuid` to keep the visible buffer in sync and uses `dot.status.dirtier_termline` to throttle winbar renders.
