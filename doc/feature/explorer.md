...
- Subscribe to `eve.state.git.refresh` notifications via a lightweight dirtier (`dot.status.dirtier_explorer_git`). Whenever git cache refreshes, iterate the visible UUIDs and update `git_display` / `git_highlight` using `eve.state.git.resolve_status(filepath, filetype)`.
