...
- Persists to `std.path.locate_workspace_filepath("notepad.json")`. The snapshot tracks items, their display order, and the active UUID.
...
- Guarantees at least one item exists (allocates an untitled note on startup) and normalises blank names to `eve.setting.BUF_UNTITLED`.
- Auto-saves with a 10 s `std.timer.debounce` and exposes `save/flush` helpers for manual persistence.
...
- Provides interactive prompts for create/rename/destroy flows and reports results through `std.reporter`.
...
- `Notepad.flush_to_disk()` creates parent directories when needed and emits a `std.reporter.info` notification on success.
