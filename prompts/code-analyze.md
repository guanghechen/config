...
After completing the review, save a copy of the full review output to `{cwd}/.code-analyze/{simple-title}-codex.md`, where `{simple-title}` is a concise kebab-case title derived from the review target (e.g., `user-auth-service`, `payment-controller`).
