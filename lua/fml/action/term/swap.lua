---@class fml.action.term.swap
local M = {}

---@param step                          integer|nil
---@return nil
function M.swap_left(step)
  local index_current, termuuid_current = dot.term.current() ---@type integer, string|nil
  if termuuid_current == nil then
    return
  end

  step = math.max(1, step or vim.v.count1 or 1)
  local N = dot.term.size() ---@type integer
  local index_next = ark.fn.navigate_circular(index_current, -step, N) ---@type integer
  local termuuid_next = dot.term.at(index_next) ---@type string|nil

  if termuuid_next == nil or termuuid_next == termuuid_current then
    return
  end

  dot.term.put(index_current, termuuid_next)
  dot.term.put(index_next, termuuid_current)
  dot.state.status.dirtier_termline:mark_dirty()
end

---@param step                          integer|nil
---@return nil
function M.swap_right(step)
  local index_current, termuuid_current = dot.term.current() ---@type integer, string|nil
  if termuuid_current == nil then
    return
  end

  step = math.max(1, step or vim.v.count1 or 1)
  local N = dot.term.size() ---@type integer
  local index_next = ark.fn.navigate_circular(index_current, step, N) ---@type integer
  local termuuid_next = dot.term.at(index_next) ---@type string|nil

  if termuuid_next == nil or termuuid_next == termuuid_current then
    return
  end

  dot.term.put(index_current, termuuid_next)
  dot.term.put(index_next, termuuid_current)
  dot.state.status.dirtier_termline:mark_dirty()
end

return M
