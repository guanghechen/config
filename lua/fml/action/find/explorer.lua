...
local function preview_render(composer, bufnr)
  ...
  if fileitem.type == "file" then
    local is_text_file = std.filetype.is_printable_file(fileitem.name) ---@type boolean
  ...
  keymaps_finder = {
    {
      ...
      callback = function()
        ...
        if item ~= nil then
          eve.ai.add_locations_to_ai({ { filepath = item.data.fileitem.path } })
        end
      end,
    },
  },
