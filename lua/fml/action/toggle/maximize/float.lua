...
---@param original                      eve.state.maximized.IOriginalFloatWindow|nil
---@return boolean
local function restore(original)
  if original == nil then
    eve.state.maximized.clear_original_float()
    return false
  end
...
function M.maximize(winnr)
  local original = eve.state.maximized.get_original_float() ---@type eve.state.maximized.IOriginalFloatWindow|nil
  if original ~= nil then
  ...
  eve.state.maximized.set_original_float({
    winnr = winnr,
    winblend = winblend,
  ...
  local cfg = eve.state.maximized.compute_float_maximized_wincfg(wincfg) ---@type vim.api.keyset.win_config
  local ok = pcall(vim.api.nvim_win_set_config, winnr, cfg) ---@type boolean
  if not ok then
    restore(eve.state.maximized.get_original_float())
    return
  end
