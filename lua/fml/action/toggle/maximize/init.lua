...
---@return nil
function M.maximize()
  local winnr = era.state.status.get_winnr_command() ---@type integer|nil
  if winnr == nil or winnr < 1 or not vim.api.nvim_win_is_valid(winnr) then
    return
  end

  local original_normal = era.state.maximized.get_original_normal() ---@type era.state.maximized.IOriginalNormalWindow|nil
