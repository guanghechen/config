...
function M.insert_splitline()
  ...
  local content = type(item.content) == "string" and item.content or "" ---@type string
  local lines = vim.split(content, "\n", { plain = true, trimempty = false }) ---@type string[]
  if #lines == 0 then
    lines = { "" }
  end
  local total_lines = #lines ---@type integer

  local cursor_row ---@type integer|nil
  local bufnr = widget:get_bufnr() ---@type integer|nil
  ...
  local insert_index = math.min(cursor_row, total_lines) ---@type integer
  local new_lines = {} ---@type string[]

  for i = 1, insert_index do
    new_lines[#new_lines + 1] = lines[i] or ""
  end

  vim.list_extend(new_lines, { "", eve.buf.CONTENT_SPLITLINE, "" })

  for i = insert_index + 1, total_lines do
    new_lines[#new_lines + 1] = lines[i]
  end

  local new_content = table.concat(new_lines, "\n") ---@type string
  if not widget:set_content(nil, new_content) then
    return
  end
  ...
  if bufnr ~= nil and vim.api.nvim_buf_is_valid(bufnr) then
    ...
    if winnr ~= nil then
      ...
      pcall(vim.api.nvim_win_set_cursor, winnr, { target_row, 0 })
    end
  end
end
