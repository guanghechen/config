local Textarea = require("fml.ux.textarea")

---@class fml.ux.IInput
---@field public bufnr                  integer|nil
---@field public winnr                  integer|nil
---@field public open                   fun(self: fml.ux.IInput, params: fml.ux.input.IOpenParams): nil
---@field public close                  fun(self: fml.ux.IInput): nil

---@class fml.ux.input.IOpenParams
---@field public initial_value          string
---@field public row                    ?number
---@field public col                    ?number
---@field public width                  ?number
---@field public max_width              ?number
---@field public min_width              ?number
---@field public text_cursor_col        ?integer
---@field public win_cursor_row         ?integer
---@field public win_cursor_col         ?integer

---@class fml.ux.Input : fml.ux.IInput, fml.ux.Textarea
local M = {}
M.__index = M
setmetatable(M, Textarea)

---@class fml.ux.input.IProps
---@field public position               eve.e.BoxPosition
---@field public width                  ?number
---@field public title                  ?string
---@field public max_width              ?number
---@field public min_width              ?number
---@field public keymaps                ?eve.t.IKeymap[]
---@field public win_opts               ?table<string, any>
---@field public validate               ?fun(value: string): string|nil
---@field public on_close               ?fun(): nil
---@field public on_confirm             fun(value: string): boolean

---@param props                         fml.ux.input.IProps
---@return fml.ux.Input
function M.new(props)
  local position = props.position ---@type eve.e.BoxPosition
  local width = props.width ---@type number|nil
  local max_width = props.max_width ---@type number|nil
  local min_width = props.min_width ---@type number|nil
  local title = props.title ---@type string|nil

  ---@type table<string, any>
  local win_opts = vim.tbl_extend("force", {
    cursorline = false,
    number = false,
    relativenumber = false,
    signcolumn = "yes",
  }, props.win_opts or {})

  local validate_from_props = props.validate ---@type (fun(value: string): string)|nil
  local on_close_from_props = props.on_close ---@type (fun(): nil)
  local on_confirm_from_props = props.on_confirm ---@type fun(text: string): boolean

  local self
  ---@cast self                         fml.ux.Input

  ---@return nil
  local handle_close = function()
    self.on_close()
  end

  ---@return nil
  local handle_confirm = function()
    self.on_confirm()
  end

  ---@type eve.t.IKeymap[]
  local keymaps = {
    { modes = { "n" }, key = "<esc>", desc = "input: quit", callback = handle_close },
    { modes = { "i" }, key = "<cr>", desc = "input: confirm", callback = handle_confirm },
  }
  vim.list_extend(keymaps, props.keymaps or {})

  ---@param lines                       string[]
  ---@return string|nil
  local function validate(lines)
    local text = lines[1] ---@type string
    if type(validate_from_props) == "function" then
      return validate_from_props(text)
    end
  end

  ---@param lines                       string[]
  ---@return boolean
  local function on_confirm(lines)
    local text = lines[1] ---@type string
    return on_confirm_from_props(text)
  end

  local textarea = Textarea.new({
    position = position,
    width = width,
    height = 1.1,
    max_width = max_width,
    max_height = 1.1,
    min_width = min_width,
    min_height = 1.1,
    title = title,
    filetype = "text",
    keymaps = keymaps,
    win_opts = win_opts,
    validate = validate,
    on_close = on_close_from_props,
    on_confirm = on_confirm,
  })

  self = setmetatable(textarea, M)
  ---@cast self                         fml.ux.Input

  return self
end

---@param params                        fml.ux.input.IOpenParams
---@return nil
function M:open(params)
  local text = params.initial_value ---@type string

  ---@type fml.ux.textarea.IOpenParams
  local opts = {
    initial_lines = { text },
    row = params.row,
    col = params.col,
    width = params.width or vim.api.nvim_strwidth(text) + 10,
    max_width = params.max_width,
    min_width = params.min_width,
    text_cursor_col = params.text_cursor_col,
    win_cursor_col = params.win_cursor_col,
    win_cursor_row = params.win_cursor_row,
  }
  Textarea.open(self, opts)
end

return M
