  ...
  if wanted.fold then
    ...
    if info and info.level > 0 then
      if info.lines > 0 then
        ---@type fml.dressing.statuscolumn.ISign
        local sign = { type = "fold", text = dot.icon.fillchars.foldclose, texthl = "Folded" }
        signs[#signs + 1] = sign
      elseif config.folds.open and info.start == lnum then
        ---@type fml.dressing.statuscolumn.ISign
        local sign = { type = "fold", text = dot.icon.fillchars.foldopen }
        signs[#signs + 1] = sign
      end
    end
  end
...
local function statuscolumn()
  ...
  components[1] = vim.b[bufnr].fml_statuscolumn_left ~= false and components[1] or ""
  components[3] = vim.b[bufnr].fml_statuscolumn_right ~= false and components[3] or ""
