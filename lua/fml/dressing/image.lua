--- https://github.com/folke/snacks.nvim/blob/72ffb3d1a2812671bb3487e490a3b1dd380bc234/lua/snacks/image.lua#L1
...
---@class fml.dressing.image.IEnvironment
---@field public name                   string
---@field public env                    table<string, string|true>
---@field public supported              ?boolean default: false
---@field public placeholders           ?boolean default: false
---@field public transform              ?fun(data: string): string
---@field public detected               ?boolean
---@field public remote                 ?boolean this is a remote client, so full transfer of the image data is required

---@class fml.dressing.image.IConfig
---@field public enabled                ?boolean    enable image viewer
---@field public wo                     ?vim.wo|{}  options for windows showing the image
---@field public bo                     ?vim.bo|{}  options for the image buffer
...
---@class fml.dressing.image.IOptions
...
---@type fml.dressing.image.IEnvironment[]
local environments = {
  {
    name = "kitty",
  ...
  },
  {
    name = "wezterm",
    env = {
      GUANGHECHEN_IMAGE_WEZTERM = true,
      TERM = "wezterm",
      WEZTERM_PANE = true,
  ...
  {
    name = "zellij",
    env = { TERM = "zellij", ZELLIJ = true },
    supported = false,
    placeholders = false,
  },
  {
    name = "ssh",
    env = { SSH_CLIENT = true, SSH_CONNECTION = true },
    remote = true,
  },
}

M._env = nil ---@type fml.dressing.image.IEnvironment?
...
local dimensions = {} ---@type table<string, fml.dressing.image.Size>
local _id = 30
local _pid = 0
local nvim_id = 0
local uv = vim.uv or vim.loop
local images = {} ---@type table<string, fml.dressing.Image>
local have_magick ---@type boolean?

local positions = {} ---@type table<number, string>
...
function Image:convert()
  ...
  local fin = src .. "[0]"
  local root = vim.fn.stdpath("cache") .. "/guanghechen/image"
  vim.fn.mkdir(root, "p")
  src = root .. "/" .. vim.fn.fnameescape(fin) .. ".png"
...
function M.env()
  ...
  for _, e in ipairs(environments) do
    local override = os.getenv("GUANGHECHEN_IMAGE_" .. e.name:upper())
    if override then
...
---@field public img                    fml.dressing.Image
---@field public id                     number image placement id
---@field public ns                     number
---@field public buf                    number
---@field public opts                   fml.dressing.image.IOptions
---@field public augroup                number
---@field public closed                 ?boolean
---@field public extmark_id             ?number
---@field public _loc                   ?fml.dressing.image.Loc
---@field public _state                 ?snacks.image.State
local Placement = {}
Placement.__index = Placement

---@param bufnr                           integer
---@param opts                            ?fml.dressing.image.IOptions
---@return fml.dressing.image.Placement
function Placement.new(bufnr, src, opts)
  assert(type(bufnr) == "number", "`Image.new`: buf should be a number")
...
---@return integer[]
function Placement:winnrs()
  local winnrs = vim.api.nvim_tabpage_list_wins(0) ---@type integer[]
  local ret = {} ---@type integer[]
  for _, winnr in ipairs(winnrs) do
    if vim.api.nvim_win_get_buf(winnr) == self.buf then
      table.insert(ret, winnr)
    end
  end
  return ret
end
...
function Placement:render_fallback(state)
  for _, winnr in ipairs(state.winnrs) do
    ...
    M.set_cursor({ pos[1] + 1 + border_top, pos[2] + border_left })
    M.request({
      a = "p",
      i = self.img.id,
      p = self.id,
...
function Placement:state()
  ...
  for _, win in ipairs(self:winnrs()) do
    ...
    if is_fallback then
      ...
      if z >= zindex or (zindex > 0 and z > 0) then
        winnrs[#winnrs + 1] = win -- use if higher z-index or both are floating
      end
    else
      winnrs[#winnrs + 1] = win
    end
  ...
  local img_width, img_height = M.measure(self.img.file)
  ...
  ---@field loc fml.dressing.image.Loc
  ---@field winnrs number[]
  return {
...
function M.measure(file)
  file = vim.fs.normalize(file)
  if dimensions[file] then
    return dimensions[file].width, dimensions[file].height
  end
  ...
  dimensions[file] = { width = width, height = height }
  return width, height
end

---@param ev                            ?{ buf: integer, event: string }
---@return nil
function M.setup(ev)
  if did_setup then
    return
...
---@param bufnr                         integer
---@param opts                          ?fml.dressing.image.IOptions|{src?: string}
---@return fml.dressing.image.Placement|nil
function M.attach(bufnr, opts)
  local file = opts and opts.src or vim.api.nvim_buf_get_name(bufnr)
  if not M.supports(file) then
    ...
    return Placement.new(bufnr, file, opts)
  end
end
...
function M.markdown(bufnr)
  ...
  if vim.b[bufnr].snacks_image_attached then
    return
  end
  ...
  local group = vim.api.nvim_create_augroup("snacks.image.markdown." .. bufnr, { clear = true })
  ...
  local function find(from, to)
    local ret = {} ---@type { id: string, pos: fml.dressing.image.Pos, src: string }[]
    parser:for_each_tree(function(tstree)
      if not tstree then
        return
      end
      for _, node, _ in query:iter_captures(tstree:root(), bufnr, from and from - 1 or nil, to and to - 1 or nil) do
        local src = vim.treesitter.get_node_text(node, bufnr)
  ...
  local function update()
    ...
    for _, i in ipairs(find()) do
      local img = imgs[i.id]
      if not img then
        local opts = vim.tbl_extend("force", { pos = i.pos, inline = true }, config.markdown)
        img = Placement.new(bufnr, i.src, opts)
        imgs[i.id] = img
      else
        img:update()
      end
...
vim.api.nvim_create_autocmd("BufReadCmd", {
  once = true,
  pattern = "*." .. table.concat(config.formats, ",*."),
  group = fn.augroup("fml.dressing.image"),
  callback = function(e)
    M.setup(e)
  end,
})
