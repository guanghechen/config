...
function M.clear(task)
  era.state.status.searching:next(true)
end
...
function M.history_show(task)
  ...
  if bufnr == nil or not vim.api.nvim_buf_is_valid(bufnr) then
    ...
    vim.bo[bufnr].filetype = dot.filetype.UX_MESSAGE_HISTORY
...
function M.show(task)
  ...
  if KIND_MAP.CHANGES[kind] == true then
    era.state.status.msg_changes:next(message)
  end
...
function M.showcmd(task)
  ...
  era.state.status.msg_command:next(text)
...
function M.ruler(task)
  ...
  if contents == nil or #contents < 1 then
    era.state.status.msg_command:next("")
    era.state.status.dirtier_statusline:mark_dirty()
    return
  end
...
function M.showmode(task)
  ...
  era.state.status.msg_mode:next(text)
end
