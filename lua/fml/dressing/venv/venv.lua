...
local utils = require("fml.dressing.venv.utils")
...
function M.load(action)
  ...
  if ready_for_new_search == false then
    reporter.info({
      ...
      subject = "load",
      message = "Cannot start a new search while old one is running.",
    })
    return
  end
...
function M.set_venv_and_system_paths(venv_row)
  local python_name = config.get_python_name() ---@type string
  local python_parent_path = config.get_python_parent_path() ---@type string

  local venv_path = venv_row.value
  local new_bin_path
  local venv_python
  if python_parent_path:len() == 0 then
    ...
    venv_python = new_bin_path .. env.PATH_SEP .. python_name
  else
    new_bin_path = venv_path .. env.PATH_SEP .. python_parent_path
    venv_python = new_bin_path .. env.PATH_SEP .. python_name
  end
  ...
  if vim.fn.executable(venv_python) == 0 then
    reporter.info({
      ...
      message = "The python path '" .. venv_python .. "' doesnt exist.",
    })
    return
  end
  ...
  if prev_bin_path ~= nil then
    current_system_path = string.gsub(current_system_path, utils.escape_pattern(prev_bin_path .. env.PATH_ENV_SEP), "")
  end
...
function M.find_parent_venvs(parent_dir)
  ...
  if stdout == nil or stderr == nil then
    reporter.error({
      from = __module_name__,
      subject = "find_parent_venvs",
      message = "Failed to create pipes for fd process.",
    })
    return
  end
  ...
  vim.loop.read_start(stdout, select.on_read)
end
...
function M.find_workspace_venvs()
  ...
  if search_path_string:len() ~= 0 then
    ...
    select.add_lines(openPop:lines(), "Workspace")
    openPop:close()
  end
end
...
function M.find_venv_manager_venvs()
  ...
  if search_path_string:len() ~= 0 then
    local cmd = "fd . -HItd -tl --absolute-path --max-depth 1 --color never "
      .. search_path_string
      .. " --exclude '3.*.*'"
    local openPop = assert(io.popen(cmd, "r"))
    select.add_lines(openPop:lines(), "VenvManager")
    openPop:close()
