---@class guanghechen.core.action.window.IHistoryItem
---@field public bufnr number
---@field public filepath string

---@class guanghechen.core.action.window.IHistoryItemEntry
---@field public display string
---@field public ordinal string
---@field public item guanghechen.core.action.window.IHistoryItem
...
---@param x guanghechen.core.action.window.IHistoryItem
---@param y guanghechen.core.action.window.IHistoryItem
---@return integer
...
function M.back()
  ...
  local previous = history:back(1) ---@type guanghechen.core.action.window.IHistoryItem|nil
...
function M.forward()
  ...
  local next = history:forward(1) ---@type guanghechen.core.action.window.IHistoryItem|nil
  if next ~= nil then
...
function M.find_history(opts)
  ...
  local entries = {} ---@type guanghechen.core.action.window.IHistoryItemEntry[]
  ...
  if unique then
    ...
    for item, item_index in history:iterator_reverse() do
      ---@cast item guanghechen.core.action.window.IHistoryItem
      ...
      if not visited[relative_filepath] then
        ...
        if present_filepath == item.filepath then
          display_text = ghc.ui.icons.ui.Separator .. " " .. relative_filepath
        ...
        ---@type guanghechen.core.action.window.IHistoryItemEntry
        local entry = {
    ...
    for item, item_index in history:iterator_reverse() do
      ...
      if present_index == item_index then
        display_text = ghc.ui.icons.ui.Separator .. " " .. tostring(item_index) .. " " .. relative_filepath
        default_lnum = #entries + 1
      else
