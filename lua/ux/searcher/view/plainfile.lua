...
function M:calc_preview_data(context)
  ...
  if flag_replace then
    ...
    if preview_result == nil then
      ...
      local fallback_lines = dot.fs.read_file_as_lines({ filepath = filepath, silent = true }) or {} ---@type string[]
    ...
    lines = dot.fs.read_file_as_lines({ filepath = filepath, silent = true }) or {} ---@type string[]
    highlights = {} ---@type ux.searcher.IPlainfileViewHighlight[]
