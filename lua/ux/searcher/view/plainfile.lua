...
---@field public highlights             ux.searcher.IPlainfileViewHighlight[]
---@field public title                  string

---@class ux.searcher.IPlainfileViewProps
---@field public name                   string

---@class ux.searcher.PlainfileView
---@field public fullname               string
---@field public nsnr                   integer
---@field protected _disposed           boolean
---@field protected _last_bufnr         integer|nil
---@field protected _last_data          ux.searcher.IPlainfileViewData|nil
local M = {}
M.__index = M

---@param props                         ux.searcher.IPlainfileViewProps
---@return ux.searcher.PlainfileView
...
function M:calc_preview_data(context)
  ...
  if not dot.filetype.is_printable_file(filename) then
    ...
    ---@type ux.searcher.IPlainfileViewHighlight[]
    local highlights = { { offset = -1, lnum = 1, coll = 0, colr = -1, hlname = "f_sr_error" } }

    ---@type ux.searcher.IPlainfileViewData
    local result = {
  ...
  local highlights = {} ---@type ux.searcher.IPlainfileViewHighlight[]
  ...
  if flag_replace then
    ...
    if preview_result == nil then
      ...
      local fallback_lines = era.fs.read_file_as_lines({ filepath = filepath, silent = true }) or {} ---@type string[]
    ...
    lines = era.fs.read_file_as_lines({ filepath = filepath, silent = true }) or {} ---@type string[]
    highlights = {} ---@type ux.searcher.IPlainfileViewHighlight[]
...
function M:render(context, bufnr, filepath, force)
  ...
  local data = self._last_data ---@type ux.searcher.IPlainfileViewData|nil
  if force or data == nil or data.filepath ~= filepath then
    data = self:calc_preview_data(context) ---@type ux.searcher.IPlainfileViewData
