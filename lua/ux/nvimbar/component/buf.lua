...
local fn_active_buf = dot.G.register_anonymous_fn(function(bufnr)
  era.command.execute(era.command.definitions.buf.open.uuid, tostring(bufnr))
end) or ""

---@type string
local fn_focus_left_buf = dot.G.register_anonymous_fn(function()
  era.command.execute(era.command.definitions.buf.focus_left.uuid)
end) or ""

---@type string
local fn_focus_right_buf = dot.G.register_anonymous_fn(function()
  era.command.execute(era.command.definitions.buf.focus_right.uuid)
end) or ""
...
local function resolve_disambiguations(bufs)
  ...
  for index = 1, N, 1 do
    local item1 = rd_bufs[index] ---@type ux.nvimbar.component.buf.IBufItem
    local dp1 = item1.meta.dirpath_pieces ---@type string[]
    local D1 = #dp1 ---@type integer

    if index > 1 then
      local item0 = rd_bufs[index - 1] ---@type ux.nvimbar.component.buf.IBufItem
    ...
    if index + 1 < N then
      local item2 = rd_bufs[index + 1] ---@type ux.nvimbar.component.buf.IBufItem
    ...
    if depth > 0 then
      ...
      local dirpath = D1 >= 1 and table.concat(dp1, dot.env.PATH_SEP, d < 1 and 1 or d, D1) or "" ---@type string
      disambiguated[item1.bufnr] = dirpath ~= dot.env.PATH_SEP and dirpath .. dot.env.PATH_SEP or dirpath ---@type string
    end
    depth = next_depth
  end
...
---@class ux.nvimbar.component.buf
local M = {}
...
function M.bufs(position)
  ...
  local function render_bufc(buf, index, total, disambiguated_paths)
    ...
    if count_error > 0 then
      local text = " " .. dot.icon.diagnostic.Error_alt .. " " .. count_error ---@type string
    ...
    if count_warn > 0 then
      local text = " " .. dot.icon.diagnostic.Warning_alt .. " " .. count_warn ---@type string
    ...
    if count_hint > 0 and slots < 2 then
      local text = " " .. dot.icon.diagnostic.Hint_alt .. " " .. count_hint ---@type string
      text_diagnostic = text_diagnostic .. text ---@type string
      hl_text_diagnostic = hl_text_diagnostic .. txt(text, hln_bufc_hint)
      slots = slots + 1
    end
    if count_info > 0 and slots < 2 then
      local text = " " .. dot.icon.diagnostic.Information_alt .. " " .. count_info ---@type string
      text_diagnostic = text_diagnostic .. text ---@type string
    ...
    local text_order = total < 2 and "" or (dot.icon.todigit_subscript(index) .. ".") ---@type string
  ...
  local function render_buf(buf, index, order, marker, disambiguated_paths)
    ...
    local slots = 0 ---@type integer
    if count_error > 0 then
      text_diagnostic = text_diagnostic .. " " .. dot.icon.diagnostic.Error_alt .. " " .. count_error
      slots = slots + 1
    end
    if count_warn > 0 then
      text_diagnostic = text_diagnostic .. " " .. dot.icon.diagnostic.Warning_alt .. " " .. count_warn
      slots = slots + 1
    end
    if count_hint > 0 and slots < 2 then
      text_diagnostic = text_diagnostic .. " " .. dot.icon.diagnostic.Hint_alt .. " " .. count_hint
      slots = slots + 1
    end
    if count_info > 0 and slots < 2 then
      text_diagnostic = text_diagnostic .. " " .. dot.icon.diagnostic.Information_alt .. " " .. count_info
      slots = slots + 1
    end
    ...
    local text_order = dot.icon.todigit_subscript(order) .. marker ---@type string
    local text_icon = fileicon .. " " ---@type string
    local text_title = filename ---@type string
    local text_mod = is_mod and "  " or "  " ---@type string
    local text_pinned = is_mod and "  " or "  " ---@type string
  ...
  local component = {
    ...
    render = function(context, remain_width)
      ...
      if left_remain_count > 0 then
        ...
        local omitter_text = " " .. dot.icon.ui.Left .. "  " .. tostring(count) .. " " ---@type string
      ...
      if right_remain_count > 0 then
        ...
        local omitter_text = "▏" .. tostring(count) .. " " .. dot.icon.ui.Right .. "  " ---@type string
        local omitter_text_hl = txt("▏", hln_buf_omitter_sep)
          .. txt(tostring(count) .. " " .. dot.icon.ui.Right .. "  ", hln_buf_omitter) ---@type string
