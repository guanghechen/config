local __module_name__ = "ux.widget.ai.picker" ---@type string

local config = require("ux.widget.ai.config")
local state = require("ux.widget.ai.state")

---@class ux.widget.ai.picker.IItem : ux.picker.composer.list.IItem
---@field public data                   any

---@class ux.widget.ai.picker
local M = {}
...
---@param items                         ux.picker.composer.list.IItem[]
---@param width                         integer
...
---@class ux.widget.ai.picker.IAttachItemInfo
---@field public item                   ux.widget.ai.ISelectItem
---@field public category               ux.widget.ai.ItemCategory
---@field public agent_label            string
---@field public identifier             string|nil
---@field public pane_cwd               string|nil
---@field public uuid                   string

---@param info                          ux.widget.ai.picker.IAttachItemInfo
---@return string
...
---@param a                             ux.widget.ai.picker.IAttachItemInfo
---@param b                             ux.widget.ai.picker.IAttachItemInfo
---@return boolean
...
---@param items                         ux.widget.ai.ISelectItem[]
---@return ux.widget.ai.picker.IItem[], integer
local function build_attach_picker_items(items)
  local action = require("ux.widget.ai.action")
  local tmux = require("ux.widget.ai.tmux")
  ...
  for _, item in ipairs(items) do
    ...
    local category ---@type ux.widget.ai.ItemCategory
    ...
    ---@type ux.widget.ai.picker.IAttachItemInfo
    local info = {
      item = item,
      category = category,
  ...
  for _, info in ipairs(sorted_infos) do
    ...
    local icon = info.category == "attached" and std.icon.status.attached
      or (item.type == "running" and std.icon.status.detached or " ")
...
---@param attached                      ux.widget.ai.IAttachedSource[]
---@return ux.widget.ai.picker.IItem[], integer
local function build_attached_picker_items(attached)
  ...
  ---@class ux.widget.ai.picker.IAttachedItemInfo
  ---@field public source               ux.widget.ai.IAttachedSource
  ---@field public agent_label          string
  ---@field public identifier           string|nil
  ---@field public pane_cwd             string|nil

  local item_infos = {} ---@type ux.widget.ai.picker.IAttachedItemInfo[]
  ...
  for _, info in ipairs(item_infos) do
    ...
    local text_identifier = info.identifier and ("  " .. std.string.pad_end(info.identifier, width_identifier, " "))
      or ""
    local text_pane_cwd = info.pane_cwd and ("  " .. info.pane_cwd) or ""
    local text = std.icon.status.attached .. " " .. text_agent .. text_identifier .. text_pane_cwd
...
---@class ux.widget.ai.picker.IShowAttachParams
---@field public on_select              fun(item: ux.widget.ai.ISelectItem): nil
---@field public on_toggle              ?fun(item: ux.widget.ai.ISelectItem): nil

---@param params                        ux.widget.ai.picker.IShowAttachParams
---@return nil
function M.show_attach(params)
  ...
  local function do_toggle()
    ...
    if item then
      ---@cast item ux.widget.ai.picker.IItem
      on_toggle(item.data)
...
---@param attached                      ux.widget.ai.IAttachedSource[]
---@param on_select                     fun(item: ux.widget.ai.IAttachedSource): nil
---@return nil
...
---@param attached                      ux.widget.ai.IAttachedSource[]
---@param on_select                     fun(items: ux.widget.ai.IAttachedSource[]): nil
---@return nil
function M.show_send_target(attached, on_select)
  ...
  if has_multiple then
    local send_to_all_text = std.icon.status.broadcast .. " send to all (" .. #attached .. " agents)"
  ...
  local source_map = {} ---@type table<string, ux.widget.ai.IAttachedSource>
  ...
  local function confirm_selection()
    ...
    local results = {} ---@type ux.widget.ai.IAttachedSource[]
...
---@param on_select                     fun(prompt: ux.widget.ai.IPrompt, result: ux.widget.ai.IPromptRenderResult): nil
---@return nil
function M.show_prompt(on_select)
  local prompt_mod = require("ux.widget.ai.prompt")
  local ctx = prompt_mod.get_ctx()

  local picker_items = {} ---@type ux.widget.ai.picker.IItem[]
  local itemmap = {} ---@type table<string, ux.widget.ai.IPrompt>
  local result_map = {} ---@type table<string, ux.widget.ai.IPromptRenderResult>
