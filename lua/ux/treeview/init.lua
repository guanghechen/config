...
function Data:_finish_page(event, work, ok, result)
  ...
  elseif not ok or type(result) ~= "table" or result.error then
    local error = type(result) == "table" and result.error or tostring(result)
    error = error or "Provider must return {records, done}"
...
function M.new_data(options)
  ...
  local data = setmetatable({
    _native = native.new_data(options.limits),
