...
local function decorate(view, first, last)
  ...
  for row = math.max(first, cache.first), math.min(last - 1, cache.last - 1) do
    ...
    if rows.marked[index] then
      overlay(view, row, start, rows.full[index] and glyphs.selected or glyphs.self_selected, "TreeviewSelection")
    end
    ...
    if icon and icon ~= "" then
      overlay(view, row, start + 2, icon, highlight)
    end
    ...
    if rows.right_texts[index] then
      vim.api.nvim_buf_set_extmark(view.bufnr, namespace, row, 0, {
        ephemeral = true,
        virt_text = { { rows.right_texts[index], "TreeviewRightText" } },
