...
local ICONS_DIRECTORY = {
  ...
  unknown                 = { glyph = "󰉋", hl = "MiniIconsAzure"  },
...
local ICONS_EXTENSION = {
  ...
  jpeg                    = { glyph = "󰈥", hl = "MiniIconsBlue"   },
  jpg                     = { glyph = "󰈥", hl = "MiniIconsBlue"   },
  png                     = { glyph = "󰉏", hl = "MiniIconsBlue"   },
...
local ICONS_FILE = {
  ...
  [".cspell.json"]        = { glyph = "", hl = "MiniIconsGreen"  },
...
local ICONS_LSP = {
  ...
  object                  = { glyph = "󰅩", hl = "MiniIconsGrey"   },
  ...
  unknown                 = { glyph = "󰞋", hl = "MiniIconsRed"    },
...
local ICONS_OS = {
  ...
  unknown                 = { glyph = "󰟀", hl = "MiniIconsPurple" },
...
local ICONS_CATEGORY_MAP = {
  ...
  extension = ICONS_EXTENSION,
  file      = ICONS_FILE,
  filetype  = ICONS_FILETYPE,
  lsp       = ICONS_LSP,
  os        = ICONS_OS,
  -- stylua: ignore end
}
...
---@return string
---@return boolean
function M.get_directory_icon(filepath)
  if #filepath == 0 or filepath == "/" then
    local item = ICONS_DIRECTORY.unknown
  ...
  if item == nil then
    item = ICONS_DIRECTORY.unknown
    return item.glyph, item.hl, true
  end
  return item.glyph, item.hl, false
end
...
function M.get_extension_icon(filepath, filetype)
  ...
  while item == nil do
    local dot_index = string.find(extname, "%..")
    if dot_index == nil then
      break
    end

    extname = extname:sub(dot_index + 1)
    item = ICONS_EXTENSION[extname]
  end

  if type(item) == "string" then
    item = ICONS_FILETYPE[item]
  end

  if item == nil then
    filetype = filetype or ark.filetype.detect("guanghechen." .. filepath) ---@type string|nil
    if filetype ~= nil then
      return M.get_filetype_icon(filetype)
    end

    item = ICONS_EXTENSION.unknown
    return item.glyph, item.hl, true
  end
  return item.glyph, item.hl, false
end
...
---@return string
---@return boolean
function M.get_file_icon(filepath, filetype)
  if #filepath == 0 then
    local item = ICONS_FILE.unknown
    return item.glyph, item.hl, true
  end

  local last_byte = string.byte(filepath, #filepath, #filepath) ---@type integer
  ...
  if item == nil then
    filetype = filetype or ark.filetype.detect(filename) ---@type string|nil
    if filetype ~= nil then
      local glyph, hl, is_default = M.get_filetype_icon(filetype)
      if not is_default then
        return glyph, hl, false
      end
    end

    local glyph, hl, is_default = M.get_extension_icon(filepath, filetype)
    if not is_default then
      return glyph, hl, false
    end

    item = ICONS_FILE.unknown
  end
  return item.glyph, item.hl, false
end
...
function M.get_filetype_icon(filetype)
  local item = ICONS_FILETYPE[filetype]
  if type(item) == "string" then
    item = ICONS_FILETYPE[item]
  end
  if item == nil then
    item = ICONS_FILETYPE.unknown
    return item.glyph, item.hl, true
  end
  return item.glyph, item.hl, false
end
...
---@return string
---@return boolean
function M.get_lsp_icon(lsp_kind)
  local item = ICONS_LSP[lsp_kind]
  if type(item) == "string" then
    item = ICONS_LSP[item]
  end
  if item == nil then
    item = ICONS_LSP.unknown
    return item.glyph, item.hl, true
  end
  return item.glyph, item.hl, false
end
...
---@return string
---@return boolean
function M.get_os_icon(os_name)
  local item = ICONS_OS[os_name]
  if type(item) == "string" then
    item = ICONS_OS[item]
  end
  if item == nil then
    item = ICONS_OS.unknown
    return item.glyph, item.hl, true
  end
  return item.glyph, item.hl, false
end
...
local GET_ICON_MAP = {
  -- stylua: ignore start
  directory = M.get_directory_icon,
  extension = M.get_extension_icon,
  file      = M.get_file_icon,
  filetype  = M.get_filetype_icon,
  lsp       = M.get_lsp_icon,
  os        = M.get_os_icon,
  -- stylua: ignore end
}
