...
---@param ctx                           dot.module.explorer.action.IContext
---@return dot.module.explorer.Action
function M.new(ctx)
...
function M:add_locations_to_ai_visual()
  local ctx = self._ctx ---@type dot.module.explorer.action.IContext
  local nodes = ctx.get_visual_nodes() ---@type dot.module.explorer.Node[]
  if #nodes == 0 then
    return
  end
...
function M:collapse_or_parent()
  ...
  local root_uri = ctx.tree.o_root_uri:snapshot() ---@type string
  if uri:sub(-1) == "/" then
    local node = ctx.tree:locate(uri) ---@type dot.module.explorer.Node|nil
...
function M:copy_node()
  ...
  local selected_nodes = ctx.tree:get_selected_nodes() ---@type dot.module.explorer.Node[]
  if #selected_nodes > 0 then
    local current_mode = ctx.tree.select_mode ---@type dot.module.explorer.SelectModeEnum
...
function M:create_directory()
  ...
  vim.ui.input({ prompt = "Create directory: ", default = relative_path }, function(input)
    ...
    local resource = ctx.resource_manager:create(new_uri) ---@type dot.module.explorer.resource.INode|nil
...
---@class dot.module.explorer.action.IPreviewItem
