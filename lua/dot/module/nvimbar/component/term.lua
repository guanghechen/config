...
function M.items(position)
  ...
  local component = {
    ...
    render = function(_, remain_width)
      local entries = {} ---@type { term: dot.module.term.IMeta, index: integer }[]
      local total_terms = dot.term.state.size() ---@type integer
      for idx = 1, total_terms do
        local termuuid, termmeta = dot.term.state.at(idx) ---@type string|nil, dot.module.term.IMeta|nil
      ...
      local active_index = dot.term.state.current() ---@type integer
