...
function M.bufs(position)
  ...
  local function render_bufc(buf, index, total, disambiguated_paths)
    ...
    if count_error > 0 then
      local text = " " .. ark.icon.diagnostic.Error_alt .. " " .. count_error ---@type string
    ...
    if count_warn > 0 then
      local text = " " .. ark.icon.diagnostic.Warning_alt .. " " .. count_warn ---@type string
    ...
    if count_hint > 0 and slots < 2 then
      local text = " " .. ark.icon.diagnostic.Hint_alt .. " " .. count_hint ---@type string
      text_diagnostic = text_diagnostic .. text ---@type string
      hl_text_diagnostic = hl_text_diagnostic .. txt(text, hln_bufc_hint)
      slots = slots + 1
    end
    if count_info > 0 and slots < 2 then
      local text = " " .. ark.icon.diagnostic.Information_alt .. " " .. count_info ---@type string
      text_diagnostic = text_diagnostic .. text ---@type string
    ...
    local text_order = total < 2 and "" or (ark.icon.todigit_subscript(index) .. ".") ---@type string
  ...
  local function render_buf(buf, index, order, marker, disambiguated_paths)
    ...
    local slots = 0 ---@type integer
    if count_error > 0 then
      text_diagnostic = text_diagnostic .. " " .. ark.icon.diagnostic.Error_alt .. " " .. count_error
      slots = slots + 1
    end
    if count_warn > 0 then
      text_diagnostic = text_diagnostic .. " " .. ark.icon.diagnostic.Warning_alt .. " " .. count_warn
      slots = slots + 1
    end
    if count_hint > 0 and slots < 2 then
      text_diagnostic = text_diagnostic .. " " .. ark.icon.diagnostic.Hint_alt .. " " .. count_hint
      slots = slots + 1
    end
    if count_info > 0 and slots < 2 then
      text_diagnostic = text_diagnostic .. " " .. ark.icon.diagnostic.Information_alt .. " " .. count_info
      slots = slots + 1
    end
    ...
    local text_order = ark.icon.todigit_subscript(order) .. marker ---@type string
    local text_icon = fileicon .. " " ---@type string
    local text_title = filename ---@type string
    local text_mod = is_mod and "  " or "  " ---@type string
    local text_pinned = is_mod and "  " or "  " ---@type string
  ...
  local component = {
    ...
    render = function(context, remain_width)
      ...
      if left_remain_count > 0 then
        ...
        local omitter_text = " " .. ark.icon.ui.Left .. "  " .. tostring(count) .. " " ---@type string
      ...
      if right_remain_count > 0 then
        ...
        local omitter_text = "▏" .. tostring(count) .. " " .. ark.icon.ui.Right .. "  " ---@type string
        local omitter_text_hl = txt("▏", hln_buf_omitter_sep)
          .. txt(tostring(count) .. " " .. ark.icon.ui.Right .. "  ", hln_buf_omitter) ---@type string
