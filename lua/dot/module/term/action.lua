local __module_name__ = "dot.module.term.action" ---@type string

require("dot.module.term.types")

---@type dot.module.term.IProfile[]
local profiles = {
  { name = "shell", type = "shell", cmd = vim.o.shell },
}

---@param profile                       dot.module.term.IProfile|nil
---@return nil
...
function M.focus_left(step)
  ...
  local index_current = dot.term.state.indexof(termuuid) ---@type integer
...
function M.rename()
  ...
  local terminal_widget = dot.term.widget ---@type dot.module.term.widget
  ...
  vim.ui.input(input_opts, function(new_name)
    ...
    dot.term.state.update(termmeta, { name = new_name })
    dot.state.status.dirtier_termline:mark_dirty()
  end)
end
...
function M.swap_left(step)
  local index_current, termuuid_current = dot.term.state.current() ---@type integer, string|nil
  ...
  dot.term.state.put(index_current, termuuid_next)
  dot.term.state.put(index_next, termuuid_current)
  dot.state.status.dirtier_termline:mark_dirty()
end
