local Observable = require("eve.lib.collection.observable")

---@class eve.state.editor : eve.t.state.editor
local M = {}

---@return eve.t.state.editor.data
function M.defaults()
  ---@type eve.t.state.data.theme
  local theme = {
    theme = "gruvbox_dark",
    transparency = false,
    relativenumber = true,
  }

  ---@type eve.t.state.editor.data
  local data = {
    theme = theme,
  }
  return data
end

---@return eve.t.state.editor.data
function M.dump()
  if M.state == nil then
    error("[eve.state.editor] the state is not initialized.")
    return M.defaults()
  end

  local state = M.state ---@type eve.t.state.editor.state

  ---@type eve.t.state.data.theme
  local theme = {
    theme = state.theme.theme:snapshot(),
    transparency = state.theme.transparency:snapshot(),
    relativenumber = state.theme.relativenumber:snapshot(),
  }

  ---@type eve.t.state.editor.data
  local data = {
    theme = theme,
  }
  return data
end

---@param data                          eve.t.state.editor.data
---@return nil
function M.load(data)
  if M.state == nil then
    ---@type eve.t.state.state.theme
    local theme = {
      theme = Observable.from_value(data.theme.theme),
      transparency = Observable.from_value(data.theme.transparency),
      relativenumber = Observable.from_value(data.theme.relativenumber),
    }

    ---@type eve.t.state.editor.state
    local state = {
      theme = theme,
    }
    M.state = state
  else
    local state = M.state ---@type eve.t.state.editor.state

    ---! theme
    state.theme.theme:next(data.theme.theme)
    state.theme.transparency:next(data.theme.transparency)
    state.theme.relativenumber:next(data.theme.relativenumber)
  end
end

---@param data                          any
---@return eve.t.state.editor.data
function M.normalize(data)
  local resolved = M.defaults() ---@type eve.t.state.editor.data

  if type(data) ~= "table" then
    return resolved
  end
  ---@cast data eve.t.state.editor.data

  ---resolve theme data
  if type(data.theme) == "table" then
    if type(data.theme.theme) == "string" then
      resolved.theme.theme = data.theme.theme
    end
    if type(data.theme.transparency) == "boolean" then
      resolved.theme.transparency = data.theme.transparency
    end
    if type(data.theme.relativenumber) == "boolean" then
      resolved.theme.relativenumber = data.theme.relativenumber
    end
  end

  return resolved
end

---@param data                          eve.t.state.editor.data
---@return boolean
function M.equals(data)
  local cur = M.dump() ---@type eve.t.state.editor.data

  ---compare theme data
  if
    data.theme.theme ~= cur.theme.theme
    or data.theme.transparency ~= cur.theme.transparency
    or data.theme.relativenumber ~= cur.theme.relativenumber
  then
    return false
  end

  return true
end

return M
