...
function Meta.new(tabnr, tabtype, bufs, bufid_sourcefile)
  ...
  self.winnr_command = Observable.from_value(0)
  self.winnr_fixed = Observable.from_value(0)
  return self
end
...
function Meta:get_winnr_command()
  ...
  if eve.std.nvim.is_win_valid(winnr_command) then
...
function Meta:get_winnr_fixed()
  ...
  if eve.std.nvim.is_win_valid(winnr_fixed) then
    return winnr_fixed
  else
...
function Meta:rearrange_bufs()
  ...
  for i = 1, N, 1 do
    ...
    if buf ~= nil and eve.std.nvim.is_buf_valid(buf.bufnr) then
      bufs[k] = buf
      k = k + 1
    end
  end
...
S = {
  ...
  get = function(tabnr)
    if tabnr ~= nil and eve.std.nvim.is_tab_valid(tabnr) then
  ...
  resolve = function(tabnr)
    if tabnr == nil or not eve.std.nvim.is_tab_valid(tabnr) then
      return nil
    end
  ...
  on_win_focus = function(tabnr, winnr)
    ...
    local is_float = eve.std.nvim.is_win_floating(winnr) ---@type boolean
...
function M.dump()
  ...
  for _, tabnr in ipairs(tab_history.stack) do
    local tabid = eve.std.array.find_index(tabnrs, tabnr) ---@type integer|nil
    if tabid ~= nil then
      stack[#stack + 1] = tabid
    end
  end
