...
local function select(params)
  ...
  local on_confirm = params.on_confirm ---@type eve.ux.select.IOnConfirm
  local get_cursor = params.get_cursor ---@type (fun(): string|nil) | nil
  local get_present = params.get_present ---@type (fun(): string|nil) | nil
  local render_item = params.render_item ---@type eve.ux.select.IRenderItem | nil
  local fetch_preview_data = params.fetch_preview_data ---@type eve.ux.select.IFetchPreviewData | nil
  local patch_preview_data = params.patch_preview_data ---@type eve.ux.select.IPatchPreviewData | nil
  local last_items = nil ---@type eve.ux.select.IItem[] | nil
  ...
  local provider = {
    fetch_data = function(force)
      if force or last_items == nil then
        last_items = fetch_items() ---@type eve.ux.select.IItem[]
      end
