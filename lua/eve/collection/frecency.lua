...
function M.new(props)
  ...
  local items = props.items ---@type table<string, eve.t.collection.frecency.IItem>
...
function M:access(key)
  ...
  local item = self._items[key] ---@type eve.t.collection.frecency.IItem|nil
  if item == nil then
    item = { timestamps = { timestamp }, idx = 1 } ---@type eve.t.collection.frecency.IItem
...
---@return eve.t.collection.frecency.ISerializedData
function M:dump()
  ---@type eve.t.collection.frecency.ISerializedData
  local data = { items = self._items }
  return data
end

---@param data                          eve.t.collection.frecency.ISerializedData
---@return nil
function M:load(data)
  local items = data.items ---@type eve.t.collection.frecency.IItem[]
  self._items = items
end
