---@class eve.collection.CircularQueue : t.eve.collection.ICircularQueue
...
---@param queue                         t.eve.collection.ICircularQueue
---@return eve.collection.CircularQueue
...
function M:dequeue()
  ...
  local target = self._elements[self._start] ---@type t.eve.T|nil
  if self._size == 1 then
    self._size = 0
    self._start = 1
    self._end = 0
