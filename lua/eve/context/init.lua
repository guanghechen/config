...
function M.load(storage)
  ...
  local raw_data_client = storage.client and eve.fs.read_json({ filepath = storage.client, silent_on_bad_path = true })
    or nil
  local raw_data_session = storage.session
      and eve.fs.read_json({ filepath = storage.session, silent_on_bad_path = true })
    or nil
  local raw_data_workspace = storage.workspace
      and eve.fs.read_json({ filepath = storage.workspace, silent_on_bad_path = true })
    or nil

  local data_client = client.normalize(raw_data_client) ---@type t.eve.context.client.data
  local data_session = session.normalize(raw_data_session) ---@type t.eve.context.session.data
  local data_workspace = workspace.normalize(raw_data_workspace) ---@type t.eve.context.workspace.data

  client.load(data_client)
  session.load(data_session)
  workspace.load(data_workspace)
