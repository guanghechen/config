local __module_name__ = "era.explorer.widget" ---@type string

local Action = require("era.explorer.action")
local Tree = require("era.explorer.tree")
local View = require("era.explorer.view")
local ResourceFileManager = require("era.explorer.resource.file")
...
---@class era.explorer.widget.IFlagItem
---@field public desc                   string
---@field public callback               fun(): nil
---@field public snapshot               fun(): string, string

---@class era.explorer.widget.IProps
...
---@field public flags                  ?era.explorer.widget.IFlagItem[]
---@field public on_disposed            ?fun(): nil

---@class era.explorer.Widget : dot.t.IWidget
---@field public name                   string
---@field public fullname               string
---@field protected _action             era.explorer.Action
---@field protected _autocmd_ids        integer[]
---@field protected _bufnr              integer|nil
---@field protected _disposed           boolean
---@field protected _flags              era.explorer.widget.IFlagItem[]
---@field protected _is_focused         boolean
---@field protected _keymaps            stl.t.IKeymap[]
---@field protected _nvimbar            era.nvimbar.Nvimbar
...
---@field protected _render_result      era.explorer.view.IRenderResult|nil
---@field protected _resource_manager   era.explorer.resource.FileManager
---@field protected _subscriptions      stl.c.IUnsubscribable[]
---@field protected _tree               era.explorer.Tree
---@field protected _view               era.explorer.View
---@field protected _winnr              integer|nil
local M = {}
M.__index = M

---@param props                         era.explorer.widget.IProps
...
function M.new(props)
  ...
  }) ---@type era.explorer.resource.FileManager

  ---@type era.explorer.Tree
  local tree = Tree.new({
    name = name,
    protocol = "file://",
  ...
  local view = View.new(name) ---@type era.explorer.View
  ...
  ---@type era.explorer.action.IContext
  local action_ctx = {
    widget = self,
    tree = tree,
    resource_manager = resource_manager,
...
function M:get_cursor_uri()
  local render_result = self._render_result ---@type era.explorer.view.IRenderResult|nil
...
---@return era.explorer.view.IRenderResult|nil
...
function M:__create_nvimbar__()
  ...
  local flags = self:__get_flags__() ---@type era.explorer.widget.IFlagItem[]
  ---@type era.nvimbar.component.explorer.IFlagItem[]
  local nvimbar_flags = {}
...
function M:__get_flags__()
  local tree = self._tree ---@type era.explorer.Tree
...
function M:__get_visual_nodes__()
  ...
  local nodes = {} ---@type era.explorer.Node[]
  for lnum = start_lnum, end_lnum do
    local uri = render_result.lnum_to_uri[lnum] ---@type string|nil
    if uri ~= nil then
      local node = self._tree:locate(uri) ---@type era.explorer.Node|nil
      if node ~= nil then
        nodes[#nodes + 1] = node
      end
    end
...
function M:__goto_git_changed__(direction)
  local aggregated = era.git.state.aggregated() ---@type era.git.status.IAggregatedCache
...
function M:__render__()
  ...
  local root_node = self._tree:get_root_node() ---@type era.explorer.Node
...
---@protected
---@param root_node                     era.explorer.Node
---@return nil
...
function M:__setup_keymaps__(bufnr)
  local action = self._action ---@type era.explorer.Action
  ...
  local keymaps = {
    ...
    },
    {
      ...
      callback = function()
        self:__goto_matching_file__("prev", function(filepath)
          return era.lsp.diagnostic.has_diagnostics(filepath, nil)
        end)
      end,
    ...
    },
    {
      ...
      callback = function()
        self:__goto_matching_file__("prev", function(filepath)
          return era.lsp.diagnostic.has_diagnostics(filepath, vim.diagnostic.severity.ERROR)
        end)
      end,
    ...
    },
    {
      ...
      callback = function()
        self:__goto_matching_file__("prev", function(filepath)
          return era.lsp.diagnostic.has_diagnostics(filepath, vim.diagnostic.severity.WARN)
        end)
      end,
...
function M:__setup_subscriptions__()
  ...
  local sub_git_staged = era.git.state.o_staged_files:subscribe(
    stl.c.Subscriber.new({
      on_next = function()
        if self:isvisible() then
          self:__render__()
  ...
  local sub_git_unstaged = era.git.state.o_unstaged_files:subscribe(
    stl.c.Subscriber.new({
      on_next = function()
        if self:isvisible() then
          self:__render__()
  ...
  local sub_diagnostic = era.lsp.diagnostic.subscribe_all(
    stl.c.Subscriber.new({
      on_next = function()
        if self:isvisible() then
          self:__render__()
