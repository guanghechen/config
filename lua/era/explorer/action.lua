---@class era.explorer.Action
---@field protected _ctx                era.explorer.action.IContext
local M = {}
M.__index = M

---@param ctx                           era.explorer.action.IContext
---@return era.explorer.Action
function M.new(ctx)
...
function M:add_locations_to_ai()
  local ctx = self._ctx ---@type era.explorer.action.IContext
  local selected_nodes = ctx.tree:get_selected_nodes() ---@type era.explorer.Node[]
...
function M:add_locations_to_ai_visual()
  local ctx = self._ctx ---@type era.explorer.action.IContext
  local nodes = ctx.get_visual_nodes() ---@type era.explorer.Node[]
  if #nodes == 0 then
    return
  end
...
function M:collapse_or_parent()
  ...
  local root_uri = ctx.tree.o_root_uri:snapshot() ---@type string
  if uri:sub(-1) == "/" then
    local node = ctx.tree:locate(uri) ---@type era.explorer.Node|nil
...
function M:copy_node()
  ...
  local selected_nodes = ctx.tree:get_selected_nodes() ---@type era.explorer.Node[]
  if #selected_nodes > 0 then
    local current_mode = ctx.tree.select_mode ---@type era.explorer.SelectModeEnum
...
function M:create_directory()
  ...
  vim.ui.input({ prompt = "Create directory: ", default = relative_path }, function(input)
    ...
    local resource = ctx.resource_manager:create(new_uri) ---@type era.explorer.resource.INode|nil
...
function M:delete_selected()
  local ctx = self._ctx ---@type era.explorer.action.IContext
  local selected_nodes = ctx.tree:get_selected_nodes_toplevel() ---@type era.explorer.Node[]
  if #selected_nodes == 0 then
    stl.reporter.warn({
      from = ctx.fullname,
...
function M:jump_last_child()
  ...
  local last_child = node.children[#node.children] ---@type era.explorer.Node
...
function M:open_selected()
  ...
  local file_nodes = {} ---@type era.explorer.Node[]
  for _, node in ipairs(selected_nodes) do
...
function M:send_to_quickfix(root)
  local ctx = self._ctx ---@type era.explorer.action.IContext
  local selected_nodes = require("era.explorer.node").collect_selected(root) ---@type era.explorer.Node[]
...
---@param target_mode                   era.explorer.SelectModeEnum
---@return nil
...
---@class era.explorer.action.IPreviewItem
---@field public from                   string
---@field public to                     string
---@field public relative_part          string

---@alias era.explorer.action.TransferModeEnum
---| "move"
---| "copy"
...
function M:paste()
  local ctx = self._ctx ---@type era.explorer.action.IContext
  local select_mode = ctx.tree.select_mode ---@type era.explorer.SelectModeEnum
  ...
  local mode = select_mode == "cut" and "move" or "copy" ---@type era.explorer.action.TransferModeEnum
...
---@protected
---@param mode                          era.explorer.action.TransferModeEnum
...
function M:__transfer_selected__(mode, initial_target)
  ...
  ---@return era.explorer.action.IPreviewItem[]
  ---@return integer                     max_from_display_width
  local function build_preview_items(target_dir)
    local items = {} ---@type era.explorer.action.IPreviewItem[]
