local Node = require("era.explorer.node")

local __module_name__ = "era.explorer.tree" ---@type string
local math_floor = math.floor

---@class era.explorer.ITreeProps
---@field public name                   string
---@field public protocol               string
---@field public resource_manager       era.explorer.resource.IManager
...
---@field public select_mode            era.explorer.SelectModeEnum
---@field public ticks                  era.explorer.ITreeTicks
---@field protected _disposed           boolean
---@field protected _resource_manager   era.explorer.resource.IManager
---@field protected _root               era.explorer.Node
---@field protected _superroot          era.explorer.Node
local M = {}
M.__index = M

---@param props                         era.explorer.ITreeProps
---@return era.explorer.Tree
function M.new(props)
  ...
  local resource_manager = props.resource_manager ---@type era.explorer.resource.IManager
  local initial_root = props.initial_root ---@type string|nil
  local default_root = initial_root or dot.path.cwd_uri() ---@type string

  local superroot = Node.superroot(protocol) ---@type era.explorer.Node
...
function M:apply_copy_paste(target_parent_uri)
  ...
  local target_node = self:__locate__(target_parent_uri) ---@type era.explorer.Node|nil
  ...
  local selected_nodes = Node.collect_selected(self._root) ---@type era.explorer.Node[]
  ...
  local rm = self._resource_manager ---@type era.explorer.resource.IManager
  ...
  for _, node in ipairs(selected_nodes) do
    ...
    local clone = Node.clone(node, target_node) ---@type era.explorer.Node
...
function M:apply_cut_paste(target_parent_uri)
  ...
  for _, node in ipairs(selected_nodes) do
    ...
    local parent = node.parent ---@type era.explorer.Node|nil
    if parent == nil then
      stl.reporter.error({
        from = __module_name__,
        subject = subject,
    ...
    local existing_idx = target_node.chidxmap[node.nodename] ---@type integer|nil
    if existing_idx ~= nil then
      local existing = target_node.children[existing_idx] ---@type era.explorer.Node
  ...
  for _, node in ipairs(selected_nodes) do
    if node.parent ~= target_node then
      local parent = node.parent ---@type era.explorer.Node
      local source_uri = node.uri ---@type string
      local destination_uri = Node.calc_uri(target_node_uri, node.nodename, node.nodetype) ---@type string
...
function M:attach(uri)
  self:__health__()
  local rm = self._resource_manager ---@type era.explorer.resource.IManager
  local resource = rm:locate(uri) ---@type era.explorer.resource.INode|nil
  ...
  local node = self:__insert__(uri, resource) ---@type era.explorer.Node
  node.expanded = true
  self._root = node
  self.ticks.structure = self.ticks.structure + 1
  return true
...
function M:clear()
  ...
  local superroot = Node.superroot(rootname) ---@type era.explorer.Node
  self._superroot = superroot
  self._root = superroot
  self.select_mode = "select"
  self.ticks.structure = self.ticks.structure + 1
...
function M:expand_path(uri)
  ...
  local root = self._root ---@type era.explorer.Node
  ...
  local node = root ---@type era.explorer.Node
...
---@return era.explorer.resource.IManager
function M:get_resource_manager()
  return self._resource_manager
end

---@return era.explorer.Node
...
---@param resource                      era.explorer.resource.INode
---@return boolean
function M:insert(parenturi, resource)
  self:__health__()

  local rm = self._resource_manager ---@type era.explorer.resource.IManager
  local resource_node = rm:locate(parenturi) ---@type era.explorer.resource.INode|nil
  ...
  local parent = self:__insert__(parenturi, resource_node) ---@type era.explorer.Node
  self:__load_children__(parent, parenturi)

  local idx = parent.chidxmap[resource.nodename] ---@type integer|nil
  if idx ~= nil then
    local existing = parent.children[idx] ---@type era.explorer.Node
  ...
  local children = parent.children ---@type era.explorer.Node[]
  local node = Node.new(parent, resource.nodetype, resource.nodename) ---@type era.explorer.Node
...
function M:is_expanded(uri)
  self:__health__()
  local node = self:__locate__(uri) ---@type era.explorer.Node|nil
...
function M:refresh(force)
  ...
  local superroot = self._superroot ---@type era.explorer.Node
  local root = self._root ---@type era.explorer.Node
  local root_uri = root.uri ---@type string
...
---@param force_expanded                era.explorer.ForceExpandedEnum|nil
---@return nil
...
---@param uri                           string
---@param force_selected                era.explorer.ForceSelectedEnum|nil
---@return nil
...
---@protected
---@param uri                           string
---@return era.explorer.NodeTypeEnum
function M:__detect_nodetype__(uri)
  local nodetype = uri:sub(-1) == "/" and "D" or "F" ---@type era.explorer.NodeTypeEnum
  return nodetype
end

---@protected
---@param resource_manager              era.explorer.resource.IManager
---@param children                      era.explorer.Node[]
---@param candidate                     era.explorer.Node
---@return integer
...
---@protected
---@param uri                           string
---@param resource                      era.explorer.resource.INode|nil
...
function M:__insert__(uri, resource, ensure_resource)
  ...
  if resource == nil then
    if ensure_created then
      ...
      resource = rm:locate(uri) ---@type era.explorer.resource.INode|nil
      if resource == nil then
  ...
  local nodetype = resource ~= nil and resource.nodetype or self:__detect_nodetype__(uri) ---@type era.explorer.NodeTypeEnum
  ...
  local o = superroot ---@type era.explorer.Node
  ...
  for i = 1, n, 1 do
    ...
    if idx == nil then
      local child_nodetype = i == n and nodetype or "D" ---@type era.explorer.NodeTypeEnum
      local child = Node.new(o, child_nodetype, piece) ---@type era.explorer.Node
      ...
      o = o.children[idx] ---@type era.explorer.Node
    end
  end
...
function M:__load__(node, nodeindex, uri, force)
  ...
  local rm = self._resource_manager ---@type era.explorer.resource.IManager
  local resource_node = rm:locate(uri) ---@type era.explorer.resource.INode|nil
  if resource_node == nil then
...
function M:__load_children__(node, uri)
  local rm = self._resource_manager ---@type era.explorer.resource.IManager
  local items = rm:load(uri) ---@type era.explorer.resource.INode[]
  ...
  local children = node.children ---@type era.explorer.Node[]
  ...
  if unchanged and child_count > 0 then
    for index = 1, child_count, 1 do
      local child = children[index] ---@type era.explorer.Node|nil
      local item = items[index] ---@type era.explorer.resource.INode
  ...
  local new_children = {} ---@type era.explorer.Node[]
  local new_chidxmap = {} ---@type table<string, integer|nil>
  for i, item in ipairs(items) do
    local old_index = chidxmap[item.nodename] ---@type integer|nil
    local old_child = old_index ~= nil and children[old_index] or nil ---@type era.explorer.Node|nil
    if old_child == nil or old_child.nodetype ~= item.nodetype then
      local child = Node.new(node, item.nodetype, item.nodename) ---@type era.explorer.Node
