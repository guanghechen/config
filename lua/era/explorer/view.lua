local __module_name__ = "era.explorer.view" ---@type string
...
---@param bufnr                         integer
---@param tree                          era.explorer.Tree
---@param root                          era.explorer.Node
---@param options                       era.explorer.view.IRenderOptions|nil
---@return era.explorer.view.IRenderResult
function M:render(bufnr, tree, root, options)
  options = options or {} ---@type era.explorer.view.IRenderOptions

  local diag_counts = {} ---@type table<string, era.explorer.view.IDiagCounts>

  local root_uri = root.uri ---@type string

  ---@type era.explorer.view.IRenderContext
  local ctx = {
    tree = tree,
    root = root,
    root_uri = root_uri,
  ...
  local diagnostic_info_list = {} ---@type era.explorer.view.IDiagnosticInfo[]
  local git_status_list = {} ---@type era.explorer.view.IGitStatusInfo[]
  local sign_info_list = {} ---@type era.explorer.view.ISignInfo[]
  ...
  if root_is_expanded then
    ...
    local children = root.children ---@type era.explorer.Node[]
    local N = #children ---@type integer
    for i, child in ipairs(children) do
      local child_display_name = nil ---@type string|nil
  ...
  local diag_by_lnum = {} ---@type table<integer, era.explorer.view.IDiagnosticInfo>
  for _, diag_info in ipairs(diagnostic_info_list) do
    diag_by_lnum[diag_info.lnum] = diag_info
  end

  local git_by_lnum = {} ---@type table<integer, era.explorer.view.IGitStatusInfo>
  for _, git_info in ipairs(git_status_list) do
    git_by_lnum[git_info.lnum] = git_info
  end

  local sign_by_lnum = {} ---@type table<integer, era.explorer.view.ISignInfo>
  ...
  for lnum_key = 1, total_lines do
    ...
    local diag_info = diag_by_lnum[lnum_key] ---@type era.explorer.view.IDiagnosticInfo|nil
    if diag_info ~= nil then
    ...
    local git_info = git_by_lnum[lnum_key] ---@type era.explorer.view.IGitStatusInfo|nil
    if git_info ~= nil then
    ...
    local sign_info = sign_by_lnum[lnum_key] ---@type era.explorer.view.ISignInfo|nil
  ...
  ---@type era.explorer.view.IRenderResult
  return {
    lines = lines,
...
---@param bufnr                         integer
---@param render_result                 era.explorer.view.IRenderResult
---@param lnum                          integer
...
function M:update_virt_text(bufnr, render_result, lnum, cursorline_hlgroup)
  ...
  local diag_info = render_result.diag_by_lnum[lnum] ---@type era.explorer.view.IDiagnosticInfo|nil
  local git_info = render_result.git_by_lnum[lnum] ---@type era.explorer.view.IGitStatusInfo|nil
  local sign_info = render_result.sign_by_lnum[lnum] ---@type era.explorer.view.ISignInfo|nil
...
function M:__fold_empty_dirs__(node, ctx)
  ...
  while true do
    ...
    local children = current.children ---@type era.explorer.Node[]
    if #children ~= 1 then
      break
    end

    local child = children[1] ---@type era.explorer.Node
...
---@param node                          era.explorer.Node
---@param lnum                          integer
---@return era.explorer.view.IDiagnosticInfo|nil
function M:__get_diagnostic_info__(ctx, node, lnum)
  local counts = ctx.diag_counts[node.uri] ---@type era.explorer.view.IDiagCounts|nil
  if counts == nil then
    return nil
  end
...
---@param node                          era.explorer.Node
---@param lnum                          integer
---@return era.explorer.view.IGitStatusInfo|nil
function M:__get_git_status_info__(node, lnum)
  ...
  local git_text, _ = era.git.status.calc_info(filepath, filetype, 0, highlights)
...
function M:__get_node_name_highlight__(ctx, node, is_ignored, is_selected)
  ...
  if ctx.show_git_status then
    ...
    if filepath ~= "" then
      local filetype = node.nodetype == "D" and "directory" or "file" ---@type string
      local _, git_hl = era.git.status.resolve(filepath, filetype)
      if git_hl ~= nil then
        return git_hl
      end
    end
...
function M:__is_ignored__(node)
  ...
  return era.git.state.is_ignored(filepath)
end
...
function M:__precompute__(root, ctx)
  local tree = ctx.tree ---@type era.explorer.Tree
  local ticks = tree.ticks ---@type era.explorer.ITreeTicks
  local diag_counts = ctx.diag_counts ---@type table<string, era.explorer.view.IDiagCounts>
  local show_diagnostics = ctx.show_diagnostics ---@type boolean
  local bufnr_counts = {} ---@type table<integer, era.explorer.view.IDiagCounts>
  ...
  if show_diagnostics then
    ---@param node                      era.explorer.Node
    ---@return era.explorer.view.IDiagCounts
    local function compute_diagnostics(node)
      local counts = { error = 0, warn = 0, hint = 0, info = 0 } ---@type era.explorer.view.IDiagCounts
      ...
      if node.nodetype == "F" then
        ...
        if filepath ~= "" then
          ...
          if bufnr ~= nil then
            local cached = bufnr_counts[bufnr] ---@type era.explorer.view.IDiagCounts|nil
            if cached ~= nil then
              counts = cached
            else
              local diag_data = era.lsp.diagnostic.get_by_bufnr(bufnr) ---@type era.lsp.diagnostic.IBufferDiagnostics
      ...
      elseif node.nodetype == "D" and node.expanded then
        for _, child in ipairs(node.children) do
          local child_counts = compute_diagnostics(child) ---@type era.explorer.view.IDiagCounts
    ...
    if root.expanded then
      local root_counts = { error = 0, warn = 0, hint = 0, info = 0 } ---@type era.explorer.view.IDiagCounts
      for _, child in ipairs(root.children) do
        local child_counts = compute_diagnostics(child) ---@type era.explorer.view.IDiagCounts
  ...
  if #filepaths > 0 then
    era.git.state.preload_ignored(filepaths)
  end
end
...
function M:__render_node__(ctx, node, indent, lnum, display_name, is_expanded, is_selected)
  ...
  local git_info ---@type era.explorer.view.IGitStatusInfo|nil
  if ctx.show_git_status then
    git_info = self:__get_git_status_info__(node, lnum)
  end

  local diag_info ---@type era.explorer.view.IDiagnosticInfo|nil
