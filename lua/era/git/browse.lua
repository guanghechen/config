...
function M.build_fields(opts)
  ...
  local function run(args)
    local lines, code = era.git.cmd.run_sync(args, { cwd = cwd })
...
function M.get_remotes(fields)
  local lines, code = era.git.cmd.run_sync({ "remote", "-v" }, { cwd = fields.cwd })
  if code ~= 0 or type(lines) ~= "table" then
    return {}
  end
...
---@param opts                       era.git.browse.IOpenOpts|nil
function M.open(opts)
  ...
  if line_start == nil or line_end == nil then
    if vim.fn.mode():find("[vV]") then
      local s, e = stl.nvim.buf.retrieve_visual_lnum_range()
      line_start = s
      line_end = e
    else
