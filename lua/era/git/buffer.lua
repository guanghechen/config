...
---@class era.git.buffer
local M = {}

---@type table<integer, era.git.buffer.ICache>
local cache = {}

---@type era.git.Repo|nil
local repo = nil
...
---@class era.git.buffer.IUpdateLock
---@field public running              boolean
---@field public scheduled            boolean
---@field public pending_callback     (fun(): nil)|nil

---@type table<integer, era.git.buffer.IUpdateLock>
local update_locks = {}

---@param bufnr                         integer
---@return era.git.buffer.IUpdateLock
...
---@class era.git.buffer.IBatchInvalidateOpts
...
---@param opts                          era.git.buffer.IBatchInvalidateOpts
...
---@param buf_cache                  era.git.buffer.ICache
...
local function update_hunks(buf_cache, callback)
  ...
  local old_hunks = buf_cache.hunks ---@type era.git.Hunk[]|nil
  local old_hunks_staged = buf_cache.hunks_staged ---@type era.git.Hunk[]|nil
  ...
  local function finish()
    ...
    buf_cache.hunks = era.git.diff.run_diff(compare_text_index, buf_lines)

    if compare_text_index == compare_text_head then
      buf_cache.hunks_staged = nil
    else
      local hunks_head = era.git.diff.run_diff(compare_text_head, buf_lines)
      buf_cache.hunks_staged = era.git.diff.filter_common(hunks_head, buf_cache.hunks)
    end

    era.git.hunk.set(bufnr, buf_cache.hunks)

    local hunks_changed = era.git.hunk.compare_heads(buf_cache.hunks, old_hunks) ---@type boolean
    local hunks_staged_changed = era.git.hunk.compare_heads(buf_cache.hunks_staged, old_hunks_staged) ---@type boolean

    if should_force_update or hunks_changed or hunks_staged_changed then
      era.git.sign.update(bufnr, buf_cache.hunks, buf_cache.hunks_staged, {
  ...
  if need_head then
    era.git.cmd.get_show_text_async(toplevel, "HEAD:" .. relpath, function(lines)
  ...
  if need_index then
    era.git.cmd.get_show_text_async(toplevel, object_name --[[@as string]], function(lines)
...
function M.attach(bufnr, opts)
  ...
  local function do_attach(r)
    ...
    local ok = vim.api.nvim_buf_attach(bufnr, false, {
      ...
      on_lines = function(_, buf, _, first, last_orig, last_new)
        ...
        era.git.sign.on_lines(buf, last_orig, last_new)
        ...
        if bc.hunks and era.git.sign.contains_range(buf, check_start, check_end) then
          bc.force_next_update = true
        elseif bc.hunks_staged and era.git.sign.contains_range(buf, check_start, check_end) then
          bc.force_next_update = true
        end
    ...
    era.git.cmd.get_file_info_async(r.toplevel, relpath, function(file_info)
  ...
  era.git.repo.new(toplevel, function(r)
    if r then
      repo = r
      era.git.state.o_branch:next(r.abbrev_head)
      era.git.watcher.update(r.gitdir)
    end
    do_attach(r)
  end)
...
function M.detach(bufnr)
  ...
  era.git.sign.clear(bufnr)
  era.git.hunk.remove(bufnr)

  cache[bufnr] = nil
end

---@param bufnr                      integer
---@return era.git.buffer.ICache|nil
...
function M.get_hunk_at(bufnr, lnum)
  ...
  return era.git.hunk.find(lnum, buf_cache.hunks)
end

---@return era.git.Repo|nil
function M.get_repo()
  return repo
end
...
function M.refresh(bufnr, invalidate_compare_text, callback)
  ...
  if invalidate_compare_text then
    era.git.cmd.get_file_info_async(buf_cache.repo.toplevel, buf_cache.relpath, function(file_info)
      if not cache[bufnr] then
        if callback then
          callback()
        end
...
function M.reset_hunk(bufnr, range)
  ...
  local hunk ---@type era.git.Hunk|nil
  if range then
    hunk = era.git.hunk.create_partial(buf_cache.hunks, range[1], range[2])
  else
    local winnr = vim.api.nvim_get_current_win() ---@type integer
    local lnum = vim.api.nvim_win_get_cursor(winnr)[1] ---@type integer
    hunk = era.git.hunk.find(lnum, buf_cache.hunks)
  end
...
function M.stage_buffer(bufnr, callback)
  ...
  era.git.cmd.stage_file_async(buf_cache.repo.toplevel, relpath, function(ok)
    if callback then
      callback(ok)
    end
  end)
...
function M.stage_hunk(bufnr, range, callback)
  ...
  local hunks ---@type era.git.Hunk[]
  if range then
    hunks = era.git.hunk.create_partials(buf_cache.hunks, range[1], range[2])
  else
    local winnr = vim.api.nvim_get_current_win() ---@type integer
    local lnum = vim.api.nvim_win_get_cursor(winnr)[1] ---@type integer
    local hunk = era.git.hunk.find(lnum, buf_cache.hunks)
  ...
  local function do_stage()
    local patch = era.git.hunk.create_patch_multi(relpath, hunks, mode_bits, false)
    era.git.cmd.apply_patch_async(toplevel, patch, false, function(ok, err)
      if ok then
  ...
  if not buf_cache.object_name then
    era.git.cmd.add_intent_to_add_async(toplevel, relpath, function()
      era.git.cmd.get_file_info_async(toplevel, relpath, function(file_info)
        buf_cache.mode_bits = file_info and file_info.mode_bits
        buf_cache.object_name = file_info and file_info.object_name
...
---@param hunks                      era.git.Hunk[]  Staged hunks (HEAD vs Index diff)
---@return string[]
...
function M.unstage_hunk(bufnr, range, callback)
  ...
  local selected_hunks = {} ---@type era.git.Hunk[]
  ...
  if range then
    ...
    local winnr = vim.api.nvim_get_current_win() ---@type integer
    local lnum = vim.api.nvim_win_get_cursor(winnr)[1] ---@type integer
    local hunk = era.git.hunk.find(lnum, hunks_staged)
    if hunk then
  ...
  era.git.cmd.hash_object_async(toplevel, relpath, new_index_lines, function(hash)
    if not hash then
      if callback then
    ...
    era.git.cmd.update_index_async(toplevel, mode_bits, hash, relpath, function(ok)
      if ok then
        M.refresh(bufnr, true, function()
          if callback then
            callback(true, nil)
