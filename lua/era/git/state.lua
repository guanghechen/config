...
---@type era.git.status.IAggregatedCache
...
local function do_refresh()
  ...
  current_collect_cancel = era.git.status.collect_async({ base = "HEAD" }, function(workspace, status_table)
    current_collect_cancel = nil

    if type(status_table) == "table" then
      local aggregated = era.git.status.aggregate(workspace, status_table)
...
function M.status_async(base, callback)
  return era.git.status.collect_async({ base = base }, function(workspace, status_table_result)
    local result = {}
...
---@return table<string, era.git.StatusEntry>
...
function M.refresh_user_info()
  ...
  local workspace = dot.path.workspace() ---@type string
  era.git.cmd.run_async({ "config", "user.name" }, { cwd = workspace }, function(lines)
    if #lines > 0 then
      user_name = lines[1]
    end
  end)
  era.git.cmd.run_async({ "config", "user.email" }, { cwd = workspace }, function(lines)
    if #lines > 0 then
      user_email = lines[1]
    end
  end)
