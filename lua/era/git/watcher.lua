...
local function refresh_branch()
  local r = era.git.buffer.get_repo()
  if not r then
    return
  end

  era.git.cmd.get_abbrev_head_async(r.toplevel, function(abbrev_head)
    r.abbrev_head = abbrev_head
    era.git.state.o_branch:next(abbrev_head)
    era.git.state.refresh_user_info()
  end)
end
...
local function trigger_gitdir_refresh()
  ...
  debounce_timer:start(DEBOUNCE_MS, 0, vim.schedule_wrap(function()
    ...
    if do_head then
      era.git.buffer.invalidate_compare_text_all()
      era.git.state.clear_ignored_cache()
      era.git.state.refresh_async(true)
    elseif do_status then
      era.git.buffer.mark_dirty_all()
      era.git.state.refresh_async(false)
    end
  end))
end
...
local function trigger_index_refresh()
  ...
  index_debounce_timer:start(INDEX_DEBOUNCE_MS, 0, vim.schedule_wrap(function()
    ...
    era.git.buffer.invalidate_index_all()
    era.git.state.refresh_async(false)
  end))
end
...
local function init_watcher()
  ...
  era.git.repo.new(workspace, function(r)
    if r then
      era.git.state.o_branch:next(r.abbrev_head)
      era.git.state.refresh_user_info()
      M.update(r.gitdir)
      era.git.state.refresh_async()
    end
  end)
end
