...
---@type table<integer, era.git.Hunk[]|nil>
local buffer_hunks = {}
...
---@param a                             era.git.Hunk[]|nil
---@param b                             era.git.Hunk[]|nil
...
function M.create_partial(hunks, top, bot)
  ...
  local dominated_hunks = {} ---@type era.git.Hunk[]
  ...
  local hunk_type ---@type era.git.HunkType
...
function M.create_partials(hunks, top, bot)
  ...
  local result = {} ---@type era.git.Hunk[]
...
---@param next_hunk                     era.git.Hunk|nil
---@return era.git.Sign[]
function M.calc_signs(hunk, min_lnum, max_lnum, next_hunk)
  local signs = {} ---@type era.git.Sign[]
...
function M.calc_signs_all(hunks, min_lnum, max_lnum)
  ...
  for i, hunk in ipairs(hunks) do
    ...
    local next_hunk = hunks[i + 1] ---@type era.git.Hunk|nil
...
function M.stage(range, callback)
  ...
  if not era.git.buffer.is_attached(bufnr) then
    era.git.buffer.attach(bufnr)
  end
  era.git.buffer.stage_hunk(bufnr, range, callback)
end
...
function M.unstage(range, callback)
  ...
  era.git.buffer.unstage_hunk(bufnr, range, callback)
end
...
function M.reset(range)
  ...
  return era.git.buffer.reset_hunk(bufnr, range)
end
...
function M.stage_buffer(callback)
  ...
  era.git.buffer.stage_buffer(bufnr, function(ok)
    if ok then
      era.git.buffer.refresh(bufnr, nil, function()
        if callback then
          callback(ok)
        end
      end)
...
function M.reset_buffer()
  ...
  return era.git.buffer.reset_buffer(bufnr)
end
...
local function nav_impl(direction, include_staged)
  ...
  local unstaged = era.git.buffer.get_unstaged_hunks(bufnr) or {}
  local staged = include_staged and (era.git.buffer.get_staged_hunks(bufnr) or {}) or {}
