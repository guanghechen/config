...
---@param a                             era.git.Hunk
---@param b                             era.git.Hunk
---@return boolean
...
function M.filter_common(a, b)
  ...
  local ret = {} ---@type era.git.Hunk[]

  for _ = 1, math.max(#a, #b) + 1 do
    local a_h = a[a_i] ---@type era.git.Hunk|nil
    local b_h = b[b_i] ---@type era.git.Hunk|nil
...
function M.compute_word_diff(old_text, new_text)
  ...
  local changes = {} ---@type era.git.WordChange[]
  ...
  local merged = { changes[1] } ---@type era.git.WordChange[]
  for i = 2, #changes do
  ...
  local final = {} ---@type era.git.WordChange[]
  ...
  if #final > 1 then
    local merged2 = { final[1] } ---@type era.git.WordChange[]
    for i = 2, #final do
...
---@return era.git.LineWordDiff[]
function M.compute_hunk_word_diff(hunk)
  local result = {} ---@type era.git.LineWordDiff[]
