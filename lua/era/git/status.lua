...
---@param existing                   era.git.StageState
---@param incoming                   era.git.StageState
---@return era.git.StageState
...
---@param stage_state                era.git.StageState
...
---@param status_map                 table<string, era.git.StatusEntry>
---@param absolute_path              string
---@param relative_path              string
---@return era.git.StatusEntry
...
---@param entry                      era.git.StatusEntry
...
---@param opts                       era.git.status.ICollectOpts|nil
---@param callback                   fun(workspace: string, status_map: table<string, era.git.StatusEntry>, status_groups: table<string, table<string, boolean>>): nil
...
function M.collect_async(opts, callback)
  ...
  local status_map = {} ---@type table<string, era.git.StatusEntry>
  ...
  local cancel_fn1 = era.git.cmd.run_async({ "diff", "--staged", "--name-status", base, "--" }, { cwd = workspace }, function(lines, code)
  ...
  local cancel_fn2 = era.git.cmd.run_async({ "diff", "--name-status" }, { cwd = workspace }, function(lines, code)
    if not cancelled and code == 0 then
      for _, line in ipairs(lines) do
        local status, relative = parse_name_status_line(line)
        if status ~= nil and relative ~= nil then
  ...
  if include_untracked then
    local cancel_fn3 = era.git.cmd.run_async({ "ls-files", "--exclude-standard", "--others" }, { cwd = workspace }, function(lines, code)
      if not cancelled and code == 0 then
        for _, line in ipairs(lines) do
...
function M.resolve(filepath, filetype)
  ...
  local aggregated = era.git.state.aggregated()
...
---@param status_table               table<string, era.git.StatusEntry>
---@return era.git.status.IAggregatedCache
function M.aggregate(workspace, status_table)
  ...
  local file_stage = {} ---@type table<string, era.git.StageState>
  local file_summary = {} ---@type table<string, string|nil>
  local staged_files = {} ---@type string[]
  local status_entries = {} ---@type table<string, era.git.StatusEntry>
...
---@param aggregated                 era.git.status.IAggregatedCache
---@param dirpath                    string
---@return era.git.status.IDirInfo|nil
function M.compute_dir_status(aggregated, dirpath)
  ...
  local stage = nil ---@type era.git.StageState
  local summary = nil ---@type string|nil
