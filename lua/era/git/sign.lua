...
---@class era.git.sign.ISigns
---@field protected _ns                 integer
---@field protected _config             table<era.git.SignType, { text: string, hl: string }>
---@field protected _priority           integer
---@field protected _hl_to_type         table<string, era.git.SignType>
local Signs = {}
Signs.__index = Signs

---@param staged                        boolean|nil
---@return era.git.sign.ISigns
...
---@param bufnr                         integer
---@return table<integer, era.git.SignType>
function Signs:__get_extmarks__(bufnr)
  local result = {} ---@type table<integer, era.git.SignType>
  if not vim.api.nvim_buf_is_valid(bufnr) then
    return result
  end
...
function Signs:__update_incremental__(bufnr, new_signs, filter)
  ...
  local new_signs_map = {} ---@type table<integer, era.git.SignType>
  ...
  local to_add = {} ---@type era.git.Sign[]
...
local function on_win(bufnr, topline, botline)
  ...
  if hunks then
    local signs = era.git.hunk.calc_signs_all(hunks, top, bot)
    if untracked then
      for _, sign in ipairs(signs) do
  ...
  if hunks_staged then
    local signs = era.git.hunk.calc_signs_all(hunks_staged, top, bot)
...
---@param hunks_staged                  era.git.Hunk[]|nil
...
function M.update(bufnr, hunks, hunks_staged, opts)
  ...
  local new_signs = {} ---@type era.git.Sign[]
  if hunks and #hunks > 0 then
    new_signs = era.git.hunk.calc_signs_all(hunks, 1, line_count)
  ...
  local new_signs_staged = {} ---@type era.git.Sign[]
  if hunks_staged and #hunks_staged > 0 then
    new_signs_staged = era.git.hunk.calc_signs_all(hunks_staged, 1, line_count)
  end
