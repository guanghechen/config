...
---@type table<integer, table<integer, era.git.BlameInfo>>
local cache = {}
...
---@param output                     string
---@return table<integer, era.git.BlameInfo>
local function parse_blame_output(output)
  local result = {} ---@type table<integer, era.git.BlameInfo>
  local lines = vim.split(output, "\n", { plain = true })

  local current_sha = nil ---@type string|nil
  local current_info = nil ---@type era.git.BlameInfo|nil
  local commits = {} ---@type table<string, era.git.BlameInfo>
...
---@class era.git.blame.IInlineConfig
...
---@class era.git.blame.IInlineCache
---@field public changedtick         integer
---@field public lnum                integer
---@field public info                era.git.BlameInfo

---@type table<integer, era.git.blame.IInlineCache>
local inline_cache = {}

---@param info                       era.git.BlameInfo
---@return boolean
local function is_current_user(info)
  local user_name = era.git.state.get_user_name()
  local user_email = era.git.state.get_user_email()
...
---@param file                       string
---@param cwd                        string
---@param callback                   fun(blame: table<integer, era.git.BlameInfo>|nil)
...
local function inline_update(bufnr)
  ...
  local buf_cache = era.git.buffer.get_cache(bufnr)
  ...
  if buffer_blame and buffer_blame[lnum] then
    local info = buffer_blame[lnum] ---@type era.git.BlameInfo
  ...
  run_blame(bufnr, file, cwd, function(blame)
    vim.schedule(function()
      ...
      if blame and blame[current_lnum] then
        local info = blame[current_lnum] ---@type era.git.BlameInfo
...
local function inline_setup_autocmds()
  ...
  vim.api.nvim_create_autocmd({ "CursorMoved", "CursorMovedI" }, {
    ...
    callback = function(args)
      ...
      if era.git.buffer.is_attached(bufnr) then
...
---@class era.git.blame.IBufferConfig
---@field public formatter           string
---@field public hl_group            string
---@field public priority            integer
local buffer_config = {
...
---@param bufnr                      integer
---@param blame                      table<integer, era.git.BlameInfo>
  ...
  local proc = stl.c.Proc.new({
    ...
    on_exit = function(p, err)
      ...
      vim.schedule(function()
        ...
        local blame = parse_blame_output(output) ---@type table<integer, era.git.BlameInfo>
        if blame then
          cache[bufnr] = blame
          local winnr = vim.fn.bufwinid(bufnr) ---@type integer
