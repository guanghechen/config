...
---@field protected _components         era.m.nvimbar.IComponent[]
...
---@param raw_component                 era.m.nvimbar.IRawComponent
...
function M:place(position, raw_component, priority)
  ...
  local components = self._components ---@type era.m.nvimbar.IComponent[]
  local orders = self._orders ---@type integer[]
  local k = #components ---@type integer

  ---@type era.m.nvimbar.IComponent
  local component = {
...
function M:__render__(force)
  ...
  local context = build_context(preset_context) ---@type era.m.nvimbar.INvimbarContext|nil
  if context == nil then
    return nil
  end
  ...
  for _, order in ipairs(orders) do
    hltexts[order] = ""
    local component = components[order] ---@type era.m.nvimbar.IComponent
  ...
  for i = 1, N, 1 do
    ...
    local component = components[i] ---@type era.m.nvimbar.IComponent
    local position = component.position ---@type dot.e.NvimbarCompPosition
    if position == "left" then
      tl = tl .. hltext
    elseif position == "center" then
