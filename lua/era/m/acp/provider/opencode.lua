local __module_name__ = "era.m.acp.provider.opencode" ---@type string

local S = era.m.acp

---@class era.m.acp.provider.OpenCode : era.m.acp.IProvider
...
---@return era.m.acp.provider.OpenCode
...
function M:__handle_session_update__(update)
  ...
  elseif update.sessionUpdate == "plan" then
    -- Handle plan update
    if update.plan and opts.session then
      local plan_entries = {} ---@type era.m.acp.IPlanEntry[]

      if update.plan.entries then
        for _, entry in ipairs(update.plan.entries) do
          ---@type era.m.acp.IPlanEntry
          local plan_entry = {
            content = entry.content or "",
            priority = entry.priority or "medium",
            status = entry.status or "pending",
          }
          plan_entries[#plan_entries + 1] = plan_entry
        end
      end

      ---@type era.m.acp.IPlan
      local plan = {
        entries = plan_entries,
      }

      vim.schedule(function()
        opts.session:update_plan(plan)
      end)
    end
  end
end
