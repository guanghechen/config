local __module_name__ = "era.m.plugin.loader" ---@type string

---@class era.m.plugin.loader
...
function M.is_loaded(name)
  local state = M.plugins[name] ---@type era.m.plugin.IPluginState|nil
...
function M.__register_plugins__(specs)
  for _, spec in ipairs(specs) do
    ---@type era.m.plugin.IPluginState
    local state = {
      spec = spec,
      loaded = false,
...
function M.__install_package_loader__()
  table.insert(package.loaders, 2, function(modname)
    ...
    if plugin_name then
      local state = M.plugins[plugin_name] ---@type era.m.plugin.IPluginState|nil
...
function M.__load_plugins__(specs)
  ...
  for _, spec in ipairs(specs) do
    ...
    if is_lazy then
      ...
      local state = M.plugins[spec.name] ---@type era.m.plugin.IPluginState|nil
      if state then
        M.__load_plugin__(state)
      end
    end
