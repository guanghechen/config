...
function M.__create_buf_as_needed__(win)
  ...
  if bufnr == nil or not stl.nvim.buf.is_valid(bufnr) then
...
function M.__create_win_as_needed__(win)
  ...
  local wincfg = {
    zindex = dot.var.zindex.NOTIFIER,
    relative = "editor",
    anchor = "NE",
    col = vim.o.columns - 1,
    row = win.row,
  ...
  if winnr == nil or not stl.nvim.win.is_valid(winnr) then
    ...
    vim.w[winnr].wintype = stl.e.WinTypeEnum.NOTIFY
...
function M.__gen_winbar__(task, width)
  ...
  local hl_text = stl.nvim.fn.txt(text_left, hlname) .. "%=%=" .. stl.nvim.fn.txt(text_right, hlname)
  return hl_text
end
