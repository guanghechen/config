...
---@param winnr                         integer
---@return boolean
local function has_closed_fold(winnr)
...
---Toggle all folds in the side-by-side windows while preserving focus.
---@param left_winnr                    integer|nil
---@param right_winnr                   integer|nil
function M.toggle_all_folds(left_winnr, right_winnr)
  local winnrs = {} ---@type integer[]
  if M.is_valid_win(left_winnr) then
    winnrs[#winnrs + 1] = left_winnr
  end
  if M.is_valid_win(right_winnr) and right_winnr ~= left_winnr then
    winnrs[#winnrs + 1] = right_winnr
  end
  if #winnrs == 0 then
    return
  end
