...
local __module_name__ = "era.widget.win-history" ---@type string

---@class era.widget.win_history.IItem : era.m.picker.composer.list.IItem
---@field public data                   era.widget.win_history.IItemData

---@class era.widget.win_history.IItemData
...
local function fetch_data(winnr_sourcefile)
  ...
  local items = {} ---@type era.widget.win_history.IItem[]
  ...
  local _, present_ordinal = meta.history:present() ---@type dot.win.IFilepathHistoryItem|nil, integer|nil
  ...
  for history_item, ordinal in meta.history:iterator_reverse() do
    ...
    if bufnr ~= nil and vim.api.nvim_buf_is_valid(bufnr) then
      ...
      ---@type era.widget.win_history.IItem
      local item = {
        uuid = uuid,
...
local picker = era.m.picker.ListComposer.new({
  ...
  render_result = function(composer, bufnr, _, matches)
    ...
    local itemmap = composer._itemmap ---@type table<string, era.m.picker.composer.list.IItem>
    ---@cast itemmap                    table<string, era.widget.win_history.IItem>

    for _, match in ipairs(matches) do
      local item = itemmap[match.uuid] ---@type era.widget.win_history.IItem
    ...
    if item ~= nil then
      ...
      if winnr_sourcefile ~= nil then
        ...
        if meta ~= nil and item_index ~= nil then
          local history_item = meta.history:at(item_index) ---@type dot.win.IFilepathHistoryItem|nil
          if history_item ~= nil then
            ...
            elseif history_item.filepath ~= nil then
              local bufnr_target = dot.buf.loadfile(history_item.filepath) ---@type integer|nil
  ...
  on_refresh = function(composer)
    if last_winnr_sourcefile ~= nil and vim.api.nvim_win_is_valid(last_winnr_sourcefile) then
      local data = fetch_data(last_winnr_sourcefile) ---@type era.m.picker.composer.list.IResetData
...
function M.history()
  ...
  local data = fetch_data(winnr_sourcefile) ---@type era.m.picker.composer.list.IResetData
...
function M.history_forward()
  ...
  local bufnr_target = nil ---@type integer|nil
  local history = meta.history ---@type stl.c.History
  while true do
