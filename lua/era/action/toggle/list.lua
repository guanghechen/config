local __module_name__ = "era.action.toggle.list" ---@type string

---@class era.action.toggle.IItem
---@field public title                  string
---@field public group                  string|nil
---@field public snapshot               fun(): string, string
---@field public action                 fun(): nil

---@class era.action.toggle.IListItem : dot.module.picker.composer.list.IItem
---@field public data                   era.action.toggle.IItem
...
---@type table<string, table<string, era.action.toggle.IItem>>
local group_items = {
  flight = {},
  ["local"] = {
    fileencoding = {
...
local toggle_item_map = {} ---@type table<string, era.action.toggle.IItem>
...
do
  ...
  table.sort(toggle_item_names, function(x, y)
    local vx = toggle_item_map[x] ---@type era.action.toggle.IItem
    local vy = toggle_item_map[y] ---@type era.action.toggle.IItem
...
local function fetch_data()
  ...
  local items = {} ---@type era.action.toggle.IListItem[]

  for _, flag in ipairs(toggle_item_names) do
    local item = toggle_item_map[flag] ---@type era.action.toggle.IItem
    ...
    ---@type era.action.toggle.IListItem
    local list_item = {
      uuid = flag,
      text = text,
...
local function execute_action(picker)
  ...
  if lnum_current >= 1 then
    ...
    if item then
      ---@cast item era.action.toggle.IListItem
      item.data.action()
...
function M.list(arg)
  ...
  if toggle_item_map[flag_name] ~= nil then
    local item = toggle_item_map[flag_name] ---@type era.action.toggle.IItem
    item.action()
  else
    if dirty_data then
      local data = fetch_data()
