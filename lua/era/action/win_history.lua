...
local __module_name__ = "era.action.win_history" ---@type string

---@class era.action.win_history.IItem : dot.module.picker.composer.list.IItem
---@field public data                   era.action.win_history.IItemData

---@class era.action.win_history.IItemData
...
local function fetch_data(winnr_sourcefile)
  ...
  local items = {} ---@type era.action.win_history.IItem[]
  ...
  for history_item, ordinal in meta.history:iterator_reverse() do
    ...
    if bufnr ~= nil and vim.api.nvim_buf_is_valid(bufnr) then
      ...
      local icon, icon_hln = ark.fileicon.get_file_icon(filename) ---@type string, string
      ...
      ---@type era.action.win_history.IItem
      local item = {
        uuid = uuid,
...
local picker = dot.picker.ListComposer.new({
  ...
  render_result = function(composer, bufnr, _, matches)
    ...
    ---@cast itemmap                    table<string, era.action.win_history.IItem>

    for _, match in ipairs(matches) do
      local item = itemmap[match.uuid] ---@type era.action.win_history.IItem
  ...
  on_refresh = function(composer)
    if last_winnr_sourcefile ~= nil and vim.api.nvim_win_is_valid(last_winnr_sourcefile) then
      local data = fetch_data(last_winnr_sourcefile) ---@type dot.module.picker.composer.list.IResetData
...
function M.history()
  ...
  local data = fetch_data(winnr_sourcefile) ---@type dot.module.picker.composer.list.IResetData
