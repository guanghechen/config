local __module_name__ = "era.action.ai" ---@type string
...
function M.edit()
  ...
  local lnum_start, col_start, lnum_end, col_end = ark.vim.buf.retrieve_visual_range()
  ...
  then
    ...
    local lines = ark.vim.buf.retrieve_visual_range_lines(bufnr, lnum_start, col_start, lnum_end, col_end)
...
function M.submit_buffer()
  ...
  local text = ark.vim.buf.retrieve_split_block(winnr) ---@type string
  require("dot.module.ai.action").send_to_attached(text, true)
end

---@return nil
function M.submit_selection()
  local text = ark.vim.buf.retrieve_selected_text() or ""
  if #text > 0 then
    require("dot.module.ai.action").send_to_attached(text, true)
  end
end
