...
local __module_name__ = "era.action.lsp.reference" ---@type string

---@class era.action.lsp.reference.IItem
...
---@param callback                      fun(ok: boolean, items: era.action.lsp.reference.IItem[]|nil): nil
...
local function fetch_data(method, buf_flagname, additional_params, callback)
  ...
  vim.lsp.buf_request_all(bufnr_sourcefile, method, params, function(results_per_client)
    ...
    local items = {} ---@type era.action.lsp.reference.IItem[]
    ...
    if #items == 1 then
      local item = items[1] ---@type era.action.lsp.reference.IItem
...
local function focus(title, method, buf_flagname, additional_params)
  fetch_data(method, buf_flagname, additional_params, function(ok, items)
    ...
    local rootdir = dot.path.cwd() ---@type string
    ...
    for _, item in ipairs(items) do
      ...
      if string.sub(filepath, 1, #rootdir) ~= rootdir then
        while true do
          local parent = dot.path.dirname(rootdir) ---@type string
    ...
    vim.schedule(function()
      ...
      treeview:traverse_filenode(nil, function(node, nodestate)
        if nodestate ~= nil and nodestate.locations ~= nil then
          local locations = nodestate.locations ---@type dot.module.picker.view.filetree.ILocationNodeState[]
