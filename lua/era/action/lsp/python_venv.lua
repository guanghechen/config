...
---@class era.action.lsp.python_venv.IItemData
---@field public icon                   string
---@field public path                   string

---@class era.action.lsp.python_venv.IItem : dot.module.picker.composer.list.IItem
---@field public data                   era.action.lsp.python_venv.IItemData
...
local function fetch_data()
  ...
  local items = {} ---@type era.action.lsp.python_venv.IItem[]
      ...
      for _, line in ipairs(lines) do
        if #line > 0 then
          ...
          if not uuid_set[dirpath] then
            ...
            ---@type era.action.lsp.python_venv.IItem
            local item = {
              uuid = dirpath,
              text = text_content,
  ...
  do
    ...
    if vim.fn.isdirectory(ark.lang.python.paths.AnacondaBase) ~= 0 then
      local icon = "" ---@type string
      local dirpath = ark.string.remove_last_slash(ark.lang.python.paths.AnacondaBase .. "/") ---@type string
      if not uuid_set[dirpath] then
        uuid_set[dirpath] = true
        local text_content = icon .. " " .. dirpath
...
picker = dot.picker.ListComposer.new({
  ...
  on_confirm = function(composer, item)
    ---@cast item era.action.lsp.python_venv.IItem
...
---@class era.action.lsp.python_venv
local M = {}
