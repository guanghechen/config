...
function M:open()
  ...
  local hunk = era.git.buffer.get_hunk_at(bufnr, lnum) ---@type era.git.Hunk|nil
  local is_staged = false ---@type boolean

  if not hunk then
    local staged_hunks = era.git.buffer.get_staged_hunks(bufnr)
    if staged_hunks then
      hunk = era.git.hunk.find(lnum, staged_hunks)
      if hunk then
        is_staged = true
      end
    end
...
function M:__render__(hunk)
  ...
  local word_diffs = era.git.diff.compute_hunk_word_diff(hunk)
  local word_diff_by_old = {} ---@type table<integer, era.git.WordChange[]>
  local word_diff_by_new = {} ---@type table<integer, era.git.WordChange[]>
  ...
  if is_staged then
    keymaps[#keymaps + 1] = {
      ...
      callback = function()
        ...
        era.git.hunk.unstage(range, function(ok, err)
          if ok then
            stl.reporter.info({
              from = __module_name__,
              subject = "Git Hunk",
    ...
    keymaps[#keymaps + 1] = {
      ...
      callback = function()
        ...
        local effective_vend = hunk.vend == 0 and 1 or hunk.vend ---@type integer
        local range = { effective_start, effective_vend }
        era.git.hunk.stage(range, function(ok, err)
          if ok then
            stl.reporter.info({
              from = __module_name__,
              subject = "Git Hunk",
    ...
    keymaps[#keymaps + 1] = {
      ...
      callback = function()
        ...
        local effective_vend = hunk.vend == 0 and 1 or hunk.vend ---@type integer
        local range = { effective_start, effective_vend }
        local ok, err = era.git.hunk.reset(range)
        if ok then
          stl.reporter.info({
            from = __module_name__,
            subject = "Git Hunk",
