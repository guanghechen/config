...
local handlers = {
  ...
  msg_history_clear = function(task)
    require("era.dressing.ui_attach.messages").history_clear(task)
  end,
...
local function process_queue()
  ...
  while tasks:size() > 0 do
    local task = tasks:dequeue() ---@type era.dressing.ui_attach.ITask

    ---! Optimization: merge adjacent cmdline_hide and cmdline_show events.
    if task.event == "cmdline_hide" then
      local next_task = tasks:dequeue() ---@type era.dressing.ui_attach.ITask
