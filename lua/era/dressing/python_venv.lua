local __module_name__ = "era.dressing.python_venv" ---@type string
...
---@class era.dressing.python_venv
local M = {}
...
function M.activate_venv(venv_path)
  ...
  if prev_bin_path ~= nil then
    current_system_path = string.gsub(current_system_path, escape_pattern(prev_bin_path .. ark.env.PATH_ENV_SEP), "")
  end

  -- Add new bin path to path
  local new_system_path = bin_path .. ark.env.PATH_ENV_SEP .. current_system_path
  ...
  if ark.env.IS_WIN then
    ...
    local conda_base_path_std = string.gsub(ark.lang.python.paths.AnacondaBase, "/", "\\")
    local conda_envs_path_std = string.gsub(ark.lang.python.paths.AnacondaEnvs, "/", "\\")
...
vim.api.nvim_create_autocmd("FileType", {
  group = ark.vim.fn.augroup("filetype_python_venv"),
  pattern = "python",
  callback = setup,
})
