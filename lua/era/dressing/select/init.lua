local __module_name__ = "era.dressing.select" ---@type string

---@class era.dressing.select.IDimension
---@field public row                    ?integer
---@field public col                    ?integer
---@field public height                 ?integer
---@field public width                  ?integer

---@class era.dressing.select.IOptions
---@field public name                   ?string
---@field public prompt                 ?string
---@field public format_item            ?fun(item): string
---@field public kind                   ?string
---@field public dimension              ?era.dressing.select.IDimension
...
---@class era.dressing.select.IItemData
---@field public original_item          any

---@class era.dressing.select.IItem : dot.module.picker.composer.list.IItem
---@field public data                   era.dressing.select.IItemData

---@alias era.dressing.select.IDataProvider
---| fun(items: any[], opts: era.dressing.select.IOptions): dot.module.picker.composer.list.IResetData, integer, dot.module.picker.composer.list.IRenderResult|nil, dot.module.picker.composer.list.IRenderPreview|nil

local providers = {
  ---@type era.dressing.select.IDataProvider
  codeaction = function(items, opts)
    local provider = require("era.dressing.select.provider.codeaction")
    return provider(items, opts)
  end,
  ---@type era.dressing.select.IDataProvider
  snacks = function(items, opts)
    local provider = require("era.dressing.select.provider.snacks")
    return provider(items, opts)
  end,
  ---@type era.dressing.select.IDataProvider
  fallback = function(items, opts)
    local provider = require("era.dressing.select.provider.fallback")
    return provider(items, opts)
  end,
}

---@param opts                          era.dressing.select.IOptions
---@return era.dressing.select.IDataProvider
local function resolve_provider(opts)
  local provider = providers[opts.kind] ---@type era.dressing.select.IDataProvider|nil
...
function M.select(items, opts, on_choice)
  local provider = resolve_provider(opts) ---@type era.dressing.select.IDataProvider
  ...
  if opts.uuid_current then
    for _, item_data in ipairs(data.items) do
      ---@cast item_data era.dressing.select.IItem
  ...
  local function do_toggle()
    ...
    if item then
      ---@cast item era.dressing.select.IItem
