...
local function get_buf_signs(bufnr, wanted)
  local signs_map = {} ---@type table<integer, era.dressing.statuscolumn.ISign[]>
...
local function line_signs(winnr, bufnr, lnum, wanted)
  local buf_signs = sign_cache[bufnr] ---@type table<integer, era.dressing.statuscolumn.ISign[]>|nil
  if not buf_signs then
    buf_signs = get_buf_signs(bufnr, wanted)
    sign_cache[bufnr] = buf_signs
  end
  local signs = buf_signs[lnum] or {} ---@type era.dressing.statuscolumn.ISign[]
  ...
  if wanted.fold then
    ...
    if info and info.level > 0 then
      if info.lines > 0 then
        ---@type era.dressing.statuscolumn.ISign
        local sign = { type = "fold", text = ark.icon.fillchars.foldclose, texthl = "Folded" }
        signs[#signs + 1] = sign
      elseif config.folds.open and info.start == lnum then
        ---@type era.dressing.statuscolumn.ISign
        local sign = { type = "fold", text = ark.icon.fillchars.foldopen }
        signs[#signs + 1] = sign
      end
    end
  end
...
local function statuscolumn()
  ...
  return "%@v:lua.require'era.dressing.statuscolumn'.click_fold@" .. ret .. "%T"
end
...
vim.o.statuscolumn = "%!v:lua.require'era.dressing.statuscolumn'.statuscolumn()"
