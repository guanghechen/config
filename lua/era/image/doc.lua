local __module_name__ = "era.image.doc" ---@type string
...
---@alias era.image.transform   fun(match: era.image.match, ctx: era.image.ctx)

---@alias era.image.find        fun(matches: era.image.match[])

---@alias LinkDefinition                 {label: string, dest: string}

---@class era.image.Hover
---@field public img                     era.image.Placement
...
---@type table<string, era.image.transform>
M.transforms = {
...
---@param ctx                            era.image.ctx
---@return era.image.match|nil
...
---@param bufnr                          integer
---@param cb                             era.image.find
---@return nil
function M.find_visible(bufnr, cb)
  local ret = {} ---@type table<string, era.image.match>
...
function M.find(bufnr, cb, opts)
  ...
  local function parse_callback()
    local ret = {} ---@type era.image.match[]
    parser:for_each_tree(function(tstree, tree)
      if not tstree then
        return
      end
...
---@param cb                             fun(image_src?: string, image_pos?: era.image.Pos)
---@return nil
...
function M.hover()
  ...
  M.at_cursor(function(src)
    ...
    local s = require("era.image.state").data
    local placement = require("era.image.placement")
...
function M.attach(bufnr)
  ...
  if vim.b[bufnr][dot.var.N_IMAGE_ATTACHED] then
    return
  end
  vim.b[bufnr][dot.var.N_IMAGE_ATTACHED] = true

  local state_mod = require("era.image.state")
  local inline_mod = require("era.image.inline")
