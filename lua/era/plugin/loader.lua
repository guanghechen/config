local __module_name__ = "era.plugin.loader" ---@type string

---@class era.plugin.loader
---@field public plugins                table<string, era.plugin.IPluginState>
...
---@param name                          string
---@return era.plugin.IPluginState|nil
function M.get(name)
...
function M.is_loaded(name)
  local state = M.plugins[name] ---@type era.plugin.IPluginState|nil
...
function M.__install_package_loader__()
  table.insert(package.loaders, 2, function(modname)
    ...
    if plugin_name then
      local state = M.plugins[plugin_name] ---@type era.plugin.IPluginState|nil
...
function M.__load_plugins__(specs)
  ...
  for _, spec in ipairs(specs) do
    ...
    if is_lazy then
      ...
      local state = M.plugins[spec.name] ---@type era.plugin.IPluginState|nil
      if state then
        M.__load_plugin__(state)
      end
    end
...
function M.__load_plugin__(state)
  ...
  if spec.dependencies then
    for _, dep_name in ipairs(spec.dependencies) do
      local dep_state = M.plugins[dep_name] ---@type era.plugin.IPluginState|nil
      if dep_state then
