...
function M:entry_to_completion_item(entry, dirname, range)
  local success, result = pcall(function()
    ...
    return {
      ...
      data = { path = entry.name, full_path = dot.path.join(dirname, entry.name), type = entry.type },
    }
  end)
        ...
        if include_hidden or name:sub(1, 1) ~= "." then
          ...
          if not type or type == "unknown" then
            ...
            local stat_success, stat_result = pcall(function()
              return uv.fs_stat(dot.path.join(dirname, name))
            end)
