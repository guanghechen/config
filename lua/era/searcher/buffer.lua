...
local __module_name__ = "era.searcher.buffer" ---@type string

local c = require("era.nvimbar").component
local Nvimbar = require("era.nvimbar").Nvimbar

local NSNR_SEARCH = vim.api.nvim_create_namespace("era.searcher.buffer") ---@type integer
local NSNR_SEARCH_CURRENT = vim.api.nvim_create_namespace("era.searcher.buffer.current") ---@type integer
local NSNR_REPLACE_PREVIEW = vim.api.nvim_create_namespace("era.searcher.buffer.replace_preview") ---@type integer
...
---@param title                         string
---@return era.searcher.result.IFlagItem[], era.searcher.result.IFlagItemRaw[]
local function create_flag_items(o_flag_fuzzy, o_flag_regex, o_flag_case_sensitive, o_flag_replace, title)
  ---@type era.searcher.result.IFlagItemRaw[]
  local raw_flags = {
    {
...
---@class era.searcher.buffer.ISearcherProps
...
---@class era.searcher.buffer.Searcher : dot.t.IWidget
...
---@param props                         era.searcher.buffer.ISearcherProps|nil
---@return era.searcher.buffer.Searcher
...
function M:__create_finder_buffer_as_needed__()
  ...
  vim.fn.sign_define(sign_name, { text = stl.icon.ui.Search, texthl = "m_pk_finder_prompt" })
...
function M:__create_replacer_buffer_as_needed__()
  ...
  vim.fn.sign_define(sign_name, { text = stl.icon.symbols.flag_replace, texthl = "m_pk_replacer_prompt" })
  vim.fn.sign_place(1, sign_group, sign_name, bufnr, { lnum = 1, priority = 10 })
...
---@protected
---@param raw_flags                     era.searcher.result.IFlagItemRaw[]
...
---@param flags                         era.searcher.result.IFlagItem[]
...
function M:__update_replace_preview__()
  ...
  local flag_regex = self.o_flag_regex:snapshot() ---@type boolean
  local flag_case_sensitive = self.o_flag_case_sensitive:snapshot() ---@type boolean
  local replacement_matches = {} ---@type era.searcher.buffer.IReplacementMatch[]
...
---@class era.searcher.buffer.IReplacementMatch
...
---@protected
---@param bufnr                         integer
---@param replacement_matches           era.searcher.buffer.IReplacementMatch[]
---@return nil
