local __module_name__ = "era.term.action" ---@type string

require("era.term.types")

---@type era.term.IProfile[]
local profiles = {
  { name = "shell", type = "shell", cmd = vim.o.shell },
}

---@param profile                       era.term.IProfile|nil
---@return nil
...
local function open_yazi(name, cwd, filepath)
  ...
  local terminal = era.term.widget ---@type era.term.widget
...
function M.create()
  ...
  era.choices.open({
    ...
    on_choice = function(selected_item)
      ...
      local selected_profile = profiles[selected_index] ---@type era.term.IProfile|nil
...
---@return nil
function M.destroy()
  local termindex = era.term.state.current() ---@type integer
  local _, termmeta = era.term.state.at(termindex) ---@type string|nil, era.term.IMeta|nil
  ...
  vim.ui.input({
    ...
    vim.defer_fn(function()
      era.term.event.on_closed(termmeta)
...
function M.focus_left(step)
  local N = era.term.state.size() ---@type integer
  local termuuid = era.term.state.o_termuuid:snapshot() ---@type string
  local index_current = era.term.state.indexof(termuuid) ---@type integer
  ...
  era.term.state.focus(index_next)
end
...
function M.rename()
  ...
  local terminal_widget = era.term.widget ---@type era.term.widget
  ...
  vim.ui.input(input_opts, function(new_name)
    ...
    era.term.state.update(termmeta, { name = new_name })
    dot.state.status.dirtier_termline:mark_dirty()
  end)
end
...
function M.swap_left(step)
  local index_current, termuuid_current = era.term.state.current() ---@type integer, string|nil
  ...
  local N = era.term.state.size() ---@type integer
  local index_next = stl.fn.navigate_circular(index_current, -step, N) ---@type integer
  local termuuid_next = era.term.state.at(index_next) ---@type string|nil

  if termuuid_next == nil or termuuid_next == termuuid_current then
    return
  end

  era.term.state.put(index_current, termuuid_next)
  era.term.state.put(index_next, termuuid_current)
  dot.state.status.dirtier_termline:mark_dirty()
end
...
function M.toggle()
  ...
  local termindex = era.term.state.current() ---@type integer
  local _, termmeta_current = era.term.state.at(termindex) ---@type string|nil, era.term.IMeta|nil
  ...
  local _, termmeta_shell = era.term.state.find_index_by_type("shell") ---@type integer, era.term.IMeta|nil
