local __module_name__ = "era.term.widget" ---@type string
...
stl.fn.observe({ era.term.state.o_termuuid }, function()
  local winnr = _terminal_winnr ---@type integer|nil
...
local function create_default_keymaps(termmeta)
  ...
  keymaps[#keymaps + 1] = {
    ...
    desc = "term: close",
    callback = function()
      local _, meta = era.term.state.indexof_by_bufnr(termmeta.bufnr)
      if meta then
        era.term.event.on_closed(meta)
      end
    end,
  }
  return keymaps
...
---@class era.term.widget : dot.t.IWidget
local M = {}
...
function M:focus()
  ...
  era.term.event.on_focused(termmeta)
end
...
function M:split(direction)
  ...
  local _, termmeta = era.term.state.at(termindex) ---@type string|nil, era.term.IMeta|nil
  if termmeta == nil then
    termmeta = era.term.state.create({
      uuid = yoz.fn.uuid(),
      name = "Terminal",
      type = "shell",
      permanent = true,
...
---@param params                        era.term.IToggleAndFocusParams
---@return nil
function M:toggle_and_focus(params)
  ...
  local _, termuuid_current = era.term.state.current() ---@type integer, string|nil
  ...
  if termmeta == nil then
    ...
    era.term.state.update(termmeta, {
      name = name,
      type = typ,
      cmd = params.cmd,
      env = params.env,
    ...
    era.term.state.append(termuuid)
  end
  ...
  if autofocus then
    era.term.state.o_termuuid:next(termmeta.uuid)
  end
...
function M.__create_buf_as_needed__(termmeta)
  ...
  vim.api.nvim_create_autocmd("TermClose", {
    ...
    callback = function()
      vim.schedule(function()
        local _, _termmeta = era.term.state.indexof_by_bufnr(bufnr)
        if _termmeta then
          era.term.event.on_closed(_termmeta)
        else
