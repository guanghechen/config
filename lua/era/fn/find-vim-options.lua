local name = "era.fn.find_vim_options" ---@type string
local title = "Find Vim Options" ---@type string

---@class era.fn.find_vim_options.IItemData
...
---@class era.fn.find_vim_options.IItem : era.m.picker.composer.list.IItem
---@field public data                   era.fn.find_vim_options.IItemData
...
local function fetch_data()
  ...
  local items = {} ---@type era.fn.find_vim_options.IItem[]
  ...
  for option_name, info in pairs(vim.api.nvim_get_all_options_info()) do
    ...
    ---@type era.fn.find_vim_options.IItemData
    local data = {
      name = option_name,
      scope = info.scope,
      type = info.type,
    ...
    ---@type era.fn.find_vim_options.IItem
    local item = {
      uuid = option_name,
...
local picker = era.m.picker.ListComposer.new({
  ...
  render_result = function(_, bufnr, itemmap, matches)
    ---@cast itemmap                    table<string, era.fn.find_vim_options.IItem>
    local lines = {} ---@type string[]
    local uuids = {} ---@type string[]
    for _, match in ipairs(matches) do
      local item = itemmap[match.uuid] ---@type era.fn.find_vim_options.IItem
  ...
  on_confirm = function(composer, item)
    ...
    ---@cast item era.fn.find_vim_options.IItem
    composer:close()

    dirty_data = false

    local data = item.data ---@type era.fn.find_vim_options.IItemData
