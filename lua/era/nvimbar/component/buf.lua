---@class era.nvimbar.component.buf.IBufItem
...
---@param x                             era.nvimbar.component.buf.IBufItem
---@param y                             era.nvimbar.component.buf.IBufItem
---@return boolean
...
local rd_bufs = {} ---@type era.nvimbar.component.buf.IBufItem[]
...
local function resolve_disambiguations(bufs)
  ...
  for _, buf in ipairs(bufs) do
    ...
    if meta ~= nil then
      local item = { bufnr = buf.bufnr, meta = meta } ---@type era.nvimbar.component.buf.IBufItem
      N = N + 1
      rd_bufs[N] = item
    end
  end
  ...
  for index = 1, N, 1 do
    local item1 = rd_bufs[index] ---@type era.nvimbar.component.buf.IBufItem
    local dp1 = item1.meta.dirpath_pieces ---@type string[]
    local D1 = #dp1 ---@type integer

    if index > 1 then
      local item0 = rd_bufs[index - 1] ---@type era.nvimbar.component.buf.IBufItem
    ...
    if index + 1 < N then
      local item2 = rd_bufs[index + 1] ---@type era.nvimbar.component.buf.IBufItem
...
---@class era.nvimbar.component.buf
local M = {}
