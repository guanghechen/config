...
---@class era.nvimbar.component.term
local M = {}
...
function M.items(position)
  ...
  local component = {
    ...
    render = function(_, remain_width)
      local entries = {} ---@type { term: era.term.IMeta, index: integer }[]
      local total_terms = era.term.state.size() ---@type integer
      for idx = 1, total_terms do
        local termuuid, termmeta = era.term.state.at(idx) ---@type string|nil, era.term.IMeta|nil
      ...
      local active_index = era.term.state.current() ---@type integer
