local __module_name__ = "era.nvimbar" ---@type string

---@class era.nvimbar.INvimbarPresetContext
---@field public winnr                  ?integer

---@alias era.nvimbar.IGetNvimbarPresetContext
---| fun(): era.nvimbar.INvimbarPresetContext|nil

---@class era.nvimbar.INvimbarContext
---@field public winnr                  integer
...
---@class era.nvimbar.IItem
---@field public name                   string
---@field public position               dot.e.NvimbarCompPosition

---@class era.nvimbar.INvimbarProps
---@field public name                   string
...
---@field public get_preset_context     ?era.nvimbar.IGetNvimbarPresetContext
---@field public is_active              fun(context: era.nvimbar.INvimbarContext): boolean
---@field public on_fulfilled           ?fun(result: string): nil
---@field public validate               ?fun(): string|nil

---@class era.nvimbar.Nvimbar
...
---@field protected _components         era.nvimbar.IComponent[]
---@field protected _orders             integer[]
---@field protected _scheduler          stl.c.Scheduler
---@field protected _get_max_width      fun(): integer
---@field protected _get_preset_context era.nvimbar.IGetNvimbarPresetContext
---@field protected _isactive           fun(context: era.nvimbar.INvimbarContext): boolean
local M = {}
M.__index = M

---@param preset_context                era.nvimbar.INvimbarPresetContext
---@return era.nvimbar.INvimbarContext|nil
local function build_context(preset_context)
  ...
  local git_branch = era.git.state.get_branch() ---@type string|nil
  git_branch = git_branch ~= "" and git_branch or nil

  ---@type era.nvimbar.INvimbarContext
  local context = {
    winnr = winnr,
    bufnr = bufnr,
    cwd = cwd,
...
---@param props                         era.nvimbar.INvimbarProps
...
function M.new(props)
  ...
  local value = stl.c.Observable.from_value("") ---@type stl.c.Observable

  ---@type era.nvimbar.IGetNvimbarPresetContext
  local get_preset_context = props.get_preset_context
    or function()
      local winnr = vim.api.nvim_get_current_win() ---@type integer
      ---@type era.nvimbar.INvimbarPresetContext
      return {
        winnr = winnr,
      }
    end

  local isactive = props.is_active ---@type fun(context: era.nvimbar.INvimbarContext): boolean
...
---@param raw_component                 era.nvimbar.IRawComponent
...
function M:place(position, raw_component, priority)
  ...
  local components = self._components ---@type era.nvimbar.IComponent[]
  local orders = self._orders ---@type integer[]
  local k = #components ---@type integer

  ---@type era.nvimbar.IComponent
  local component = {
...
function M:__render__(force)
  local preset_context = self._get_preset_context() or {} ---@type era.nvimbar.INvimbarPresetContext
  local context = build_context(preset_context) ---@type era.nvimbar.INvimbarContext|nil
  if context == nil then
    return nil
  end
  ...
  for _, order in ipairs(orders) do
    hltexts[order] = ""
    local component = components[order] ---@type era.nvimbar.IComponent
  ...
  for i = 1, N, 1 do
    ...
    local component = components[i] ---@type era.nvimbar.IComponent
    local position = component.position ---@type dot.e.NvimbarCompPosition
    if position == "left" then
      tl = tl .. hltext
    elseif position == "center" then
