local __module_name__ = "era.ui_attach" ---@type string
...
function M.dressing()
  ...
  local handlers = {
    cmdline_hide = function(task)
      require("era.ui_attach.cmdline").hide(task)
    end,
    cmdline_special_char = function(task)
      require("era.ui_attach.cmdline").special_char(task)
    end,
    cmdline_pos = function(task)
      require("era.ui_attach.cmdline").pos(task)
    end,
    cmdline_show = function(task)
      require("era.ui_attach.cmdline").show(task)
    end,
    cmdline_block_show = function(task)
      require("era.ui_attach.cmdline").block_show(task)
    end,
    cmdline_block_append = function(task)
      require("era.ui_attach.cmdline").block_append(task)
    end,
    cmdline_block_hide = function(task)
      require("era.ui_attach.cmdline").block_hide(task)
    end,
    msg_clear = function(task)
      require("era.ui_attach.messages").clear(task)
    end,
    msg_history_clear = function(task)
      require("era.ui_attach.messages").history_clear(task)
    end,
    msg_history_show = function(task)
      require("era.ui_attach.messages").history_show(task)
    end,
    msg_show = function(task)
      require("era.ui_attach.messages").show(task)
    end,
    msg_showcmd = function(task)
      require("era.ui_attach.messages").showcmd(task)
    end,
    msg_showmode = function(task)
      require("era.ui_attach.messages").showmode(task)
    end,
    msg_ruler = function(task)
      require("era.ui_attach.messages").ruler(task)
    end,
    popupmenu_hide = function(task)
      require("era.ui_attach.popupmenu").hide(task)
    end,
    popupmenu_select = function(task)
      require("era.ui_attach.popupmenu").select(task)
    end,
    popupmenu_show = function(task)
      require("era.ui_attach.popupmenu").show(task)
    end,
  }
  ...
  local function process_task(task)
    local handle = handlers[task.event] ---@type era.ui_attach.IHandleTask
    handle(task)
  end
  ...
  local function process_queue()
    ...
    while tasks:size() > 0 do
      local task = tasks:dequeue() ---@type era.ui_attach.ITask

      ---! Optimization: merge adjacent cmdline_hide and cmdline_show events.
      if task.event == "cmdline_hide" then
        local next_task = tasks:dequeue() ---@type era.ui_attach.ITask
