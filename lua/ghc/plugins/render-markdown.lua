...
return {
  ...
  ft = dot.filetype.get_markdown_filetypes(),
  cmd = { "RenderMarkdown" },
  opts = {
    debounce = 200,
    file_types = dot.filetype.get_markdown_filetypes(),
    log_level = "error",
    log_runtime = false,
    max_file_size = 1,
    nested = true,
  ...
  config = function(_, opts)
    ...
    ark.fn.observe({ era.context.plugin.render_markdown }, function()
      local flag = era.context.plugin.render_markdown:snapshot() ---@type boolean
      if flag then
        plugin.enable()
      else
        plugin.disable()
