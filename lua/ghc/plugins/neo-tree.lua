...
return {
  ...
  opts = {
    ...
    commands = {
      ...
      open_ghc_file_explorer = function(state)
        ...
        local filepath = node:get_id() ---@type string
        vim.cmd(dot.command.definitions.find.explorer.uuid .. " " .. filepath)
      end,
      open_ghc_file_finder = function(state)
        ...
        if node.type == "file" then
          ...
          vim.cmd(dot.command.definitions.find.files.uuid .. " " .. dirpath)
          return
        end
      ...
      open_ghc_searcher = function(state)
        ...
        local filepath = node:get_id() ---@type string
        vim.cmd(dot.command.definitions.search.in_files.uuid .. " " .. filepath)
      end,
    ...
    event_handlers = {
      {
        ...
        handler = function(args)
          ...
          ark.nvim.bindkeys(keymaps, { bufnr = args.bufnr, noremap = true })
          vim.cmd("stopinsert")
        end,
      },
    },
  ...
  config = function(_, opts)
    ...
    vim.api.nvim_create_autocmd("TermClose", {
      group = ark.nvim.augroup("neotree_refresh_gitstatus"),
      pattern = "*lazygit",
      callback = function()
