...
local M = {
  ...
  render = function()
    ...
    --      hl_text = eve.nvim.txt(text_noice_command, "f_sl_noice_command")
    ...
    if text_noice_mode ~= nil and #text_noice_mode > 0 then
      if width > 0 then
        hl_text = hl_text .. eve.nvim.txt(" ", "f_sl_bg")
        width = width + 1
      end

      hl_text = hl_text .. eve.nvim.txt(text_noice_mode, "f_sl_noice_mode")
