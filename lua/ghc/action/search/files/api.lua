local __module_name__ = "ghc.action.search.files" ---@type string
...
---@class ghc.action.search.files.IHighlight : eve.t.IHighlight
---@field public offset                 integer

---@class ghc.action.search.files.IPreviewData
---@field public filetype               string|nil
---@field public highlights             ghc.action.search.files.IHighlight[]
---@field public lines                  string[]
---@field public title                  string

local _fileitem_map = {} ---@type table<string, ghc.action.search.files.IFileItem>
local _item_map = {} ---@type table<string, ghc.action.search.files.IItem>
local _last_preview_data = nil ---@type ghc.action.search.files.IPreviewData|nil
...
---@class ghc.action.search.files.api
local M = {}

---@param uuid                          string
---@return ghc.action.search.files.IPreviewData
---@return integer
---@return integer
function M.calc_preview_data(uuid)
  local item = _item_map[uuid] ---@type ghc.action.search.files.IItem|nil
  if item == nil then
    local lines = { "  Cannot retrieve the item by uuid=" .. uuid } ---@type string[]

    ---@type ghc.action.search.files.IHighlight[]
    local highlights = { { offset = -1, lnum = 1, coll = 0, colr = -1, hlname = "f_us_preview_error" } }

    ---@type ghc.action.search.files.IPreviewData
  ...
  local highlights = {} ---@type ghc.action.search.files.IHighlight[]
  ...
  if flag_replace then
    ...
    highlights = {} ---@type ghc.action.search.files.IHighlight[]
    ...
    if filematch ~= nil then
      ...
      for _, block_match in ipairs(filematch.matches) do
        ...
        for _, search_match in ipairs(block_match.matches) do
          ...
          if vim.tbl_contains(match_offsets, match_offset) then
...
function M.collect_valid_match_offsets(uuid)
  ...
  local fileitem = _fileitem_map[item.filepath] ---@type ghc.action.search.files.IFileItem
...
function M.fetch_data(input_text, force, callback)
  ...
  local fileitem_map = {} ---@type table<string, ghc.action.search.files.IFileItem>
  local item_map = {} ---@type table<string, ghc.action.search.files.IItem>
  for _, filepath in ipairs(result.item_orders) do
    ...
    if filematch ~= nil then
      ---@type ghc.action.search.files.IFileItem
      local fileitem = {
        children = {},
        fragmentary = false,
        filematch = filematch,
      ...
      if not is_searching_current_buf then
        ...
        ---@type ghc.action.search.files.IItem
        local item = {
          filepath = filepath,
          offset = -1,
          lnum = 1,
...
function M.fetch_preview_data(search_item)
  local preview_data, lnum, col = M.calc_preview_data(search_item.uuid) ---@type ghc.action.search.files.IPreviewData
...
function M.get_filematch(filepath)
  local fileitem = _fileitem_map[filepath] ---@type ghc.action.search.files.IFileItem|nil
...
function M.open_file(item, frecency)
  ...
  local data = _item_map and _item_map[item.uuid] ---@type ghc.action.search.files.IItem|nil
  if data ~= nil then
...
function M.patch_preview_data(search_item, last_search_item, last_data)
  local item = _item_map[search_item.uuid] ---@type ghc.action.search.files.IItem|nil
...
function M.replace_file(uuid)
  ...
  local fileitem = _fileitem_map[item.filepath] ---@type ghc.action.search.files.IFileItem|nil
  if fileitem == nil then
    return
  end
  ...
  if item.offset >= 0 then
    ...
    for i = 1, #locations, 1 do
      ...
      local child_item = _item_map[child_uuid] ---@type ghc.action.search.files.IItem
