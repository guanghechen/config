...
function M.fs_cwd()
  ...
  local toggle = ft_current == dot.filetype.NEOTREE ---@type boolean
...
---@return nil
function M.git_cwd()
  local widget = widgets.git_cwd ---@type dot.t.IWidget
...
function M.git_workspace()
  local widget = widgets.git_workspace ---@type dot.t.IWidget
  if widget:isfocused() then
    widget:close()
  else
    widget:focus()
...
---@return nil
function M.toggle()
  if eve.win.find_by_filetype(0, dot.filetype.NEOTREE) ~= nil then
    require("neo-tree.command").execute({
      action = "close",
      source = "filesystem",
    })
