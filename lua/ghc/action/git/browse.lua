local __module_name__ = "ghc.action.git" ---@type string
...
---@alias ghc.action.git.browse.TargetScope
---|"branch"
---|"file"
---|"repo"

---@class ghc.action.git.browse.IRemote
---@field public name                   string
---@field public url                    string

---@class ghc.action.git.browse
local config = {
...
---@param repo                          string
---@param what                          ghc.action.git.browse.TargetScope
...
---@param remote                        ghc.action.git.browse.IRemote
...
function M.browse()
  ...
  local remotes = {} ---@type ghc.action.git.browse.IRemote[]
  local fields = {
  ...
  local scope = fields.file and "file" or (fields.branch and "branch" or "repo") ---@type ghc.action.git.browse.TargetScope
  for _, line in ipairs(system({ "git", "-C", workspace, "remote", "-v" }, "Failed to get git remotes")) do
    ...
    if name and remote_url then
      ...
      if repo then
        ---@type ghc.action.git.browse.IRemote
        local remote = {
          name = name,
