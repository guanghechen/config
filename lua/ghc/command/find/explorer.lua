...
---@class ghc.command.find.explorer.IFileItem
...
local dir_datamap = {} ---@type table<string, ghc.command.find.explorer.IDirItem>
local file_datamap = {} ---@type table<string, ghc.command.find.explorer.IFileItem>

---@param dirpath                       string
---@param force                         boolean
---@return ghc.command.find.explorer.IDirItem
local function fetch_diritem(dirpath, force)
  local diritem = (not force) and dir_datamap[dirpath] or nil ---@type ghc.command.find.explorer.IDirItem|nil
  if diritem == nil then
    local items = {} ---@type ghc.command.find.explorer.IFileItem[]
    ...
    ---@type ghc.command.find.explorer.IDirItem
    diritem = {
      items = items,
    ...
    local provider = {
      fetch_data = function(force)
        ...
        for _, fileitem in ipairs(diritem.items) do
          ...
          local item = { group = nil, uuid = fileitem.path, text = filename } ---@type fml.t.ux.select.IItem
          table.insert(items, item)
        end
      ...
      fetch_preview_data = function(item)
        local fileitem = file_datamap[item.uuid] ---@type ghc.command.find.explorer.IFileItem|nil
        ...
        local diritem = dir_datamap[dirpath] ---@type ghc.command.find.explorer.IDirItem|nil
        if diritem == nil then
        ...
        elseif fileitem.type == "directory" then
          ...
          local c_diritem = fetch_diritem(fileitem.path, false) ---@type ghc.command.find.explorer.IDirItem
          for lnum, c_fileitem in ipairs(c_diritem.items) do
            local width = 0 ---@type integer
            local text = "" ---@type string

            local sep_perm = string.rep(" ", 2) ---@type string
            local text_perm = eve.util.pad_start(c_fileitem.perm, c_diritem.perm_width, " ") .. sep_perm
            ...
            local text_size = eve.util.pad_start(c_fileitem.size, c_diritem.size_width, " ") .. sep_size
            ...
            if not env.IS_WIN then
              ...
              local text_group = eve.util.pad_end(c_fileitem.group, c_diritem.group_width, " ") .. sep_group
            ...
            local text_date = eve.util.pad_end(c_fileitem.date, c_diritem.date_width, " ") .. sep_date
            ...
            local text_name = eve.util.pad_end(c_fileitem.name, c_diritem.name_width, " ") .. sep_name
      ...
      render_item = function(item, match)
        ...
        local text_icon = eve.util.pad_start(fileitem.icon, diritem.icon_width, " ") .. sep_icon ---@type string
        ...
        local text_name = eve.util.pad_end(filename, diritem.name_width + 1, " ") .. sep_name ---@type string
        local width_name = string.len(text_name) ---@type integer
        table.insert(highlights, {
          coll = width,
          colr = width + width_name,
        ...
        local sep_perm = string.rep(" ", 2) ---@type string
        local text_perm = eve.util.pad_start(fileitem.perm, diritem.perm_width, " ") .. sep_perm ---@type string
        local width_perm = string.len(text_perm) ---@type integer
        table.insert(highlights, {
          coll = width,
          colr = width + 1,
        ...
        local sep_size = string.rep(" ", 2) ---@type string
        local text_size = eve.util.pad_start(fileitem.size, diritem.size_width, " ") .. sep_size ---@type string
        local width_size = string.len(text_size) ---@type integer
        ...
        local sep_date = string.rep(" ", 2) ---@type string
        local text_date = eve.util.pad_end(fileitem.date, diritem.date_width, " ") .. sep_date ---@type string
        local width_date = string.len(text_date) ---@type integer
