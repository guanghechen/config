...
impl TemplateWidget for WindowIdWidget {
    fn render_template(&self, _context: &RenderContext) -> AppResult<RenderedSegment> {
        Ok(RenderedSegment {
            ...
            rich_text: "#[fg=#{@GHC_SL_BG_PILL_SESSION}]#{@GHC_SEP_ROUND_LEFT}#[fg=#{@GHC_SL_BG_PILL_SESSION}#,bg=#{@GHC_SL_FG_PILL_ICON}#,reverse]#{@GHC_SYM_SESSION} #[default]#[fg=#{@GHC_SL_FG_PILL_WINNR}] #{window_id} ".to_string(),
        })
    }
}
