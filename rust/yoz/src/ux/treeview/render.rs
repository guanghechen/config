...
fn equal_text(
    ...
    target: &Snapshot,
    new: &Row,
    equality: &mut super::text::Equality,
) -> bool {
    if old.text.is_some() || new.text.is_some() {
        return old.depth == new.depth
            && match (&old.text, &new.text) {
                (Some(a), Some(b)) => old.label == new.label && a.same_prefix(b, equality),
                (Some(path), None) => {
                    new.chain.is_none() && path.single_label() && old.label == new.label
                }
                (None, Some(path)) => {
                    old.chain.is_none() && path.single_label() && old.label == new.label
                }
...
fn gap(
    ...
    /* Streaming pages usually only insert rows. Prove the old gap is an unchanged
     * subsequence before building the general identity index and ordered anchors. */
    if span.target_end - span.target_start > span.old_end - span.old_start {
        ...
        let mut first = span.target_start;
        for (offset, row) in target
            ...
            .enumerate()
        {
            let at = span.target_start + offset;
            if previous.peek().is_some_and(|before| {
                before.id == row.id && equal_text(base, before, target, row, equality)
            }) {
                if first != at {
                    additions.push(Splice {
                        old_start: old,
                        old_end: old,
                        target_start: first,
                        target_end: at,
        ...
        if old == span.old_end {
            ...
            work.compared_rows += span.target_end - span.target_start;
        ...
        .enumerate()
    {
        ...
        if let Some((at, old)) = previous
            && at >= span.old_start
            && at < span.old_end
            && equal_text(base, old, target, row, equality)
        {
...
fn row_bytes(frame: &Snapshot, row: &Row, context: &RenderContext) -> Result<usize> {
    ...
    if let Some(text) = &row.text {
        return bytes
            .checked_add(text.bytes(&row.label))
...
impl RenderPlan {
    ...
    pub fn new(
        ...
        let mut equality = super::text::Equality::default();
        ...
        if let Some(base) = base.as_ref().filter(|base| {
            ...
            if flat_replacement {
                ...
                let mut ancestry_rows = BTreeSet::new();
                if same_layout
                    && target.previous != Some(base.id)
                    && (base.ancestry_text() || target.ancestry_text())
                {
                    let mut next = 0;
                    for &(_, start, len) in &spans {
                        ancestry_rows.extend(next..start);
                        next = start + len;
                    }
                    ancestry_rows.extend(next..target.len());
                }
                ...
                let mut text_rows = ancestry_rows;
                for id in changed {
                    if let Some(at) = target.position(id) {
                        text_rows.insert(at);
                    }
                }
                let mut covering = edits.iter().peekable();
                let mut text_edits = Vec::new();
                for at in text_rows {
                    while covering.peek().is_some_and(|edit| edit.target_end <= at) {
                        covering.next();
                    }
                    if covering.peek().is_some_and(|edit| edit.target_start <= at) {
                        continue;
                    }
                    ...
                    if let Some(old_at) = base.position(row.id) {
                        ...
                        if !equal_text(
                            base,
                            base.row(old_at).expect("base row"),
                            &target,
                            row,
                            &mut equality,
                        ) {
                            text_edits.push(Splice {
                                old_start: old_at,
                                old_end: old_at + 1,
                                target_start: at,
                                target_end: at + 1,
                ...
                let insertion_only = edits.iter().all(|edit| edit.old_start == edit.old_end);
                ...
                let written: usize = edits
                    ...
                    .sum();
                /* Bounded insertion pages should not export every already loaded line.
                 * Replacement/reordering keeps the smaller call-count threshold. */
                let call_limit = if insertion_only { 512 } else { 128 };
                if edits.len() > call_limit
                    || (target.len() > 1024 && written * 10 > target.len() * 7)
                {
                    mode = PlanMode::Reset;
    ...
    pub(crate) fn write_lines(
        ...
        for row in self.target.rows.iter_from(start).take(end - start) {
            ...
            let label = if row.chain.is_none() && row.text.is_none() {
            ...
            if let Some(path) = &row.text {
                path.append(&row.label, &mut text, &mut labels);
