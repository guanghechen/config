...
pub struct Engine {
    ...
    pub(crate) observed_reads: HashSet<NodeId>,
    pub(crate) leased_reads: BTreeMap<u64, NodeId>,
...
impl Engine {
    pub fn new(limits: Limits) -> Result<Self> {
        Ok(Self {
            ...
            observed_reads: HashSet::new(),
            leased_reads: BTreeMap::new(),
    ...
    pub(crate) fn apply_operations(
        ...
        let mut appended = std::collections::VecDeque::new();
        let mut append_base: Option<Source> = None;
        for (at, operation) in batch.operations.iter().enumerate() {
            ...
            let (old, change) = if let Some(change) = appended.pop_front() {
                (append_base.as_ref().expect("append base").clone(), change)
            } else {
                let old = source.clone();
                let count = request.map_or(0, |parent| batch.operations[at..].iter().take_while(|operation| {
                    matches!(operation, Operation::Insert { parent: Some(reference), position, .. }
                        if source.resolve(reference) == Ok(parent) && match position {
                            super::data::Position::Last => true,
                            super::data::Position::Before(anchor) => source.resolve(anchor).ok().is_some_and(|id|
                                source.node(id).is_some_and(|node| node.parent == Some(parent))),
                            _ => false,
                        })
                }).count());
                let change = if count > 1 {
                    append_base = Some(old.clone());
                    appended = source
                        .insert_children(
                            request.expect("read slot"),
                            &batch.operations[at..at + count],
                            revision,
                            &self.limits,
                        )?
                        .into();
                    appended.pop_front().expect("append changes")
                } else {
                    source.apply(operation, revision, &self.limits, request)?
                };
                (old, change)
            };
            self.memory.check()?;
                ...
                && change.kind != ChangeKind::Reorder
            {
                let actual = match change.kind {
                    ChangeKind::Completeness => {
                        let node = source
                            .node(change.node.expect("updated slot"))
                            .expect("slot node");
                        ExpectedChange::Slot {
                            node: node.id,
    ...
    pub fn dispatch(&mut self, state_id: u64, command: Command, context: Context) -> Result<Reply> {
        ...
        let changed = match command {
            ...
            Command::PrepareSources { lock, retry } => {
                ...
                if retry {
                    let failed: Vec<_> = sources
                        ...
                        .copied()
                        .filter(|id| {
                            self.query_for_slot(*id).is_none()
                                && self
                                    .source
                                    .node(*id)
                        ...
                        .collect();
                    if !failed.is_empty() {
                        task.prepare(&self.source, &sources)?;
                        let mut candidate = self.clone();
                        candidate.states.insert(state_id, entry);
                        /* Retry reads belong to this task, so unlocking can cancel their demand. */
                        let effects = candidate
                            .request_observed_children(&failed, true)?
                            .into_effects();
                        candidate.deferred_effects.extend(effects);
                        let revisions = candidate.revisions(Some(state_id));
                        candidate.memory.check()?;
                        *self = candidate;
                        return Ok(Reply::Pending { revisions, sources });
                    }
                }
                ...
                let cleanup = task.prepare(&self.source, &sources)?;
                if cleanup.is_some() {
            ...
            Command::Unselect {
                ...
                for id in successful.iter().copied() {
                    if task.source_root(id).is_none() || !seen.insert(id) {
                        return Err(Error::stale(
                            "cleanup contains an unprepared or duplicate source",
                        ));
                    }
                    if task.removed.get(&id).is_some() {
                        continue;
                    }
                    if !self.source.contains(id) {
                        return Err(Error::missing(id));
                ...
                targets.retain(|(id, _)| {
                    let mut parent = self.source.node(*id).and_then(|node| node.parent);
                    let mut path = Vec::new();
                    let found = loop {
                        let Some(id) = parent else { break false };
                        if seen.contains(&id) {
                            break true;
                        }
                        if let Some(found) = covered.get(&id) {
                            break *found;
                        }
                        path.push(id);
                        parent = self.source.node(id).and_then(|node| node.parent);
                    };
                    for id in path {
                        covered.insert(id, found);
                    }
                    !found
                });
    ...
    pub fn lock_selection(
        ...
        let task = Task::new(&self.source, &entry.state.sources(&self.source)?, deadline)?;
        let token = task.lock;
        entry.state.revision = entry.state.revision.next()?;
    ...
    pub fn authorize_task_update(
        ...
        let token = candidate.authorize(&self.source, changes)?;
        self.memory.check()?;
        *task = candidate;
        Ok(token)
    }
