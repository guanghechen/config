...
pub(crate) struct State {
    ...
    /** Opaque consumer intent, published with selection and cleared when it is precisely empty. */
...
impl State {
    ...
    pub fn demands_children(&self, source: &Source, node: NodeId) -> bool {
        source.node(node).is_some_and(|node| node.data.can_expand)
            && self.observes_children(source, node)
    }

    /** Logical demand survives a temporarily unavailable children slot. */
    pub(crate) fn observes_children(&self, source: &Source, node: NodeId) -> bool {
        ...
        if self.display.mode == Mode::Tree && !self.expansion.value(source, node).unwrap_or(false) {
            return false;
        }
    ...
    pub fn summary(&mut self, source: &Source) -> Result<Summary> {
        let summary = self.summaries.forest(source, &self.selection)?;
        if summary.is_empty() && self.locked.is_none() {
            self.selection_purpose = None;
        }
        Ok(summary)
    }
