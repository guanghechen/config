...
fn compare_paths(
    ...
    for (&a, &b) in left.iter().zip(right) {
        ...
        let siblings = source
            .child_sequence(source.node(a).expect("ordered node").parent)
            .expect("sibling group");
        return siblings
            .position(&a)
            .expect("source sibling")
            .cmp(&siblings.position(&b).expect("source sibling"));
    }
...
fn boundary_at(
    ...
    while left < right {
        ...
        let current = source_path(source, state, roots, row.folded_ids()[0])?
            .ok_or_else(|| Error::invalid("surviving row is outside the display scope"))?;
        if compare_paths(source, state, roots, &current, path).is_lt()
...
fn boundaries(
    ...
    for (_, id) in parents {
        ...
        let end = boundary_at(source, state, roots, rows, &path, true)?;
...
pub(crate) fn project(
    ...
    boundaries(source, state, &root_order, &mut rows, &boundary)?;
    let mut incoming = Vec::new();
    ...
    incoming.sort_unstable_by(|a, b| compare_paths(source, state, &root_order, &a.0, &b.0));
    ...
    for (path, id) in incoming {
        ...
        let at = boundary_at(source, state, &root_order, &rows, &path, false)?;
        ...
        let (segment, requests, count) = subtree(
            source,
            state,
            previous,
            matched,
