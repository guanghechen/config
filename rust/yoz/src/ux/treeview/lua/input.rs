...
pub(crate) fn token(value: LuaValue, prefix: char) -> Result<u64> {
...
pub(crate) fn node(value: LuaValue) -> Result<NodeId> {
...
pub(crate) fn ids(value: LuaValue) -> Result<Arc<[NodeId]>> {
...
pub(crate) fn root(value: LuaValue) -> Result<Root> {
    let value = table(value)?;
    match string(get(&value, "kind")?)?.as_ref() {
...
pub(crate) fn display(value: LuaValue) -> Result<DisplayOptions> {
    if matches!(value, LuaValue::Nil) {
    ...
    Ok(DisplayOptions {
        mode,
        sort,
        list_text: match get::<Option<String>>(&value, "list_text")?
            .as_deref()
            .unwrap_or("label")
        {
            "label" => ListText::Label,
            "ancestry" => ListText::Ancestry,
            _ => return Err(Error::invalid("unknown List text style")),
        },
...
pub(super) fn command(value: LuaValue) -> Result<Command> {
    ...
    Ok(match kind.as_ref() {
        ...
        "prepare_sources" => Command::PrepareSources {
            lock: LockToken(token(get(&input, "lock")?, 'l')?),
            retry: option_bool(&input, "retry")?.unwrap_or(false),
        },
...
pub(super) fn render_context(value: LuaValue) -> Result<RenderContext> {
    let defaults = RenderContext::default();
    if matches!(value, LuaValue::Nil) {
        return Ok(defaults);
    }
    let value = table(value)?;
    Ok(RenderContext {
        version: match get(&value, "version")? {
            LuaValue::Nil => defaults.version,
            value => token(value, 'r')?,
        },
        indent: match get(&value, "indent")? {
            LuaValue::Nil => defaults.indent,
            value => integer(value)?,
        },
        slots: match get(&value, "slots")? {
            LuaValue::Nil => defaults.slots,
            value => integer(value)?,
        },
...
pub(super) fn error(value: LuaValue) -> Result<Error> {
    match value {
        LuaValue::String(value) => Ok(Error::new(
            ErrorCode::ProviderError,
            value.to_str().map_err(invalid)?.to_string(),
        )),
        LuaValue::Table(value) => {
