...
pub(crate) fn node(id: NodeId) -> String {
    token('n', id.value())
}

pub(crate) fn error(lua: &Lua, error: &Error) -> LuaResult<LuaTable> {
...
pub(crate) fn outcome(lua: &Lua, outcome: Outcome) -> LuaResult<LuaValue> {
    match outcome {
...
pub(super) fn display(lua: &Lua, value: &DisplayOptions) -> LuaResult<LuaTable> {
    let result = lua.create_table()?;
    result.set(
        "mode",
        if value.mode == Mode::Tree {
            "tree"
        } else {
            "list"
        },
    ...
    result.set(
        "list_text",
        if value.list_text == ListText::Ancestry {
            "ancestry"
        } else {
            "label"
        },
    )?;
    result.set("pattern", value.pattern.as_ref())?;
    result.set("case_sensitive", value.case_sensitive)?;
    result.set("show_hidden", value.show_hidden)?;
    result.set("selected_only", value.selected_only)?;
    result.set("compress", value.compress)?;
    result.set("branches_first", value.branches_first)?;
    result.set(
        "sort",
        match value.sort {
            Sort::Source => "source",
...
pub(super) fn header(lua: &Lua, frame: &Snapshot) -> LuaResult<LuaTable> {
    ...
    result.set(
        "selection_purpose",
        frame.state.selection_purpose.as_deref(),
    )?;
