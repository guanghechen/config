...
/** Owned provider work executes on the same writer as ordinary Treeview commands. */
pub trait NativeAction: Send {
    fn bytes(&self) -> usize;
...
/** A native children provider consumes its own effects after source publication. */
pub trait NativeReader: Send {
...
impl Action {
    fn completion(&self) -> Option<u64> {
        match self {
            Self::Native(action) => action.completion(),
    ...
    fn bytes(&self) -> usize {
        ...
        match self {
            Self::Native(action) => action.bytes(),
            ...
            Self::Dispatch(_, command, _) => match command {
                ...
                Command::Unselect { successful, .. } => successful.len().saturating_mul(8),
                _ => 256,
            },
            _ => 256,
        }
    ...
    fn control(&self) -> bool {
        if let Self::Native(action) = self {
...
#[derive(Clone)]
pub struct Ticket(Arc<(Mutex<Option<Outcome>>, Condvar)>);
...
impl Ticket {
    ...
    fn finish(&self, value: Outcome) {
        let mut outcome = self.0.0.lock().unwrap_or_else(|poison| poison.into_inner());
        if outcome.is_none() {
            *outcome = Some(value);
            self.0.1.notify_all();
        }
    ...
    pub(crate) fn wait(&self) -> Outcome {
        let outcome = self.0.0.lock().unwrap_or_else(|error| error.into_inner());
        self.0
            .1
            .wait_while(outcome, |value| value.is_none())
...
pub struct StateStatus {
    pub(crate) native_task: Option<(LockToken, CleanupToken, u64, bool)>,
    pub revisions: Revisions,
    pub locked: bool,
    pub selection_purpose: Option<Arc<str>>,
...
#[derive(Clone)]
pub struct WeakDataHandle(Weak<Runtime>);

impl WeakDataHandle {
    pub fn upgrade(&self) -> Option<DataHandle> {
        self.0.upgrade().map(DataHandle)
    }
}
...
impl DataHandle {
    pub fn new(limits: Limits) -> Result<Self> {
        Self::with_reader(limits, None)
    }

    pub fn with_reader(limits: Limits, reader: Option<Box<dyn NativeReader>>) -> Result<Self> {
        ...
        std::thread::Builder::new()
            ...
            .spawn(move || run(engine, shared, weak, reader))
            .map_err(|error| Error::new(ErrorCode::ResourceLimit, error.to_string()))?;
        Ok(Self(runtime))
    }

    pub fn acknowledge_publication(&self, revision: Revision) {
        self.0
            .shared
            .acknowledged
    ...
    pub fn publication_acknowledged(&self, revision: Revision) -> bool {
        self.0.shared.acknowledged.load(Ordering::Acquire) >= revision.value()
    }

    pub fn downgrade(&self) -> WeakDataHandle {
        WeakDataHandle(Arc::downgrade(&self.0))
    }
    ...
    pub(crate) fn memory(&self) -> Arc<super::memory::Budget> {
        self.0.shared.memory.clone()
    }
...
fn execute(engine: &mut Engine, action: Action, runtime: &Weak<Runtime>) -> Result<Outcome> {
    ...
    let reply = match action {
        Action::Native(action) => action.apply(engine)?,
...
fn events(
    ...
    for effect in effects {
        if reader.as_mut().is_some_and(|reader| reader.effect(&effect)) {
            continue;
        }
        ...
        states: engine
            ...
            .map(|(&id, entry)| {
                (
                    id,
                    StateStatus {
                        native_task: entry.task.as_ref().and_then(|task| {
                            Some((
                                task.lock,
                                task.cleanup?,
                                task.native_job?,
                                task.valid && !task.cleaned,
                            ))
                        }),
                        revisions: engine.revisions(Some(id)),
                        locked: entry.state.locked.is_some(),
                        selection_purpose: entry.state.selection_purpose.clone(),
                        display: entry.state.display.clone(),
...
fn run(
    ...
    reader: Option<Box<dyn NativeReader>>,
) {
    if std::panic::catch_unwind(std::panic::AssertUnwindSafe(|| {
        run_inner(engine, shared.clone(), runtime, reader)
    }))
    .is_err()
    {
        stop(
...
fn run_inner(
    ...
    shared: Arc<Shared>,
    runtime: Weak<Runtime>,
    mut reader: Option<Box<dyn NativeReader>>,
) {
