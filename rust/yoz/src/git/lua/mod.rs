//! Lua boundary for the Git domain. Workers never own Lua values.
mod blame;
mod ignore;
mod staging;
mod word_diff;

use crate::git::{CODES, Entry, Info, Numstat, Options, Outcome, Snapshot, Stage, StatusJob};
use mlua::prelude::*;
    ...
    result.set(
        ...
        lua.create_function(|lua, options: LuaTable| {
            ...
            lua.create_userdata(Job(
                crate::git::start_status(options).map_err(LuaError::external)?
            ))
        })?,
    )?;
