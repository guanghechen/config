    ...
    fn t_is_descendant_cases() {
        ...
        impl IPathSet {
            fn from_base(base: String) -> Self {
                let abs_src = resolve(&base, "src", false);
                let abs_src_utils = resolve(&abs_src, "utils", false);
                let abs_other = resolve(&base, "other", false);
                let rel_src = "src".to_string();
                let rel_utils = join(&rel_src, "utils", false);
                let rel_utils_deep = join(&rel_utils, "deep", false);
        ...
        for &(from_key, to_key, expected) in CASES {
            ...
            loop {
                ...
                if attempt >= MAX_ATTEMPTS {
                    panic!(
                        "cwd changed during assertion: before={base_before}, after={base_after}"
                    );
                }
            }
        }
